...
void* fex_malloc(size_t Size) { return AllocationPointers.Malloc(Size); }
...
void fex_free(void* p) { AllocationPointers.Free(p); }
...
void* fex_calloc(size_t n, size_t size) { return AllocationPointers.Calloc(n, size); }
void* __libc_calloc(size_t n, size_t size) PREALIAS(fex_calloc);
void* calloc(size_t n, size_t size) PREALIAS(fex_calloc);

void* fex_memalign(size_t align, size_t s) { return AllocationPointers.Memalign(align, s); }
...
void* fex_realloc(void* ptr, size_t size) { return AllocationPointers.Realloc(ptr, size); }
void* __libc_realloc(void* ptr, size_t size) PREALIAS(fex_realloc);
void* realloc(void* ptr, size_t size) PREALIAS(fex_realloc);

void* fex_valloc(size_t size) { return AllocationPointers.Valloc(size); }
void* __libc_valloc(size_t size) PREALIAS(fex_valloc);
void* valloc(size_t size) PREALIAS(fex_valloc);

int fex_posix_memalign(void** r, size_t a, size_t s) { return AllocationPointers.PosixMemalign(r, a, s); }
...
void* fex_aligned_alloc(size_t a, size_t s) { return AllocationPointers.AlignedAlloc(a, s); }
void* aligned_alloc(size_t a, size_t s) PREALIAS(fex_aligned_alloc);

size_t fex_malloc_usable_size(void* ptr) { return AllocationPointers.MallocUsable(ptr); }

size_t __malloc_usable_size(void* ptr) { return fex_malloc_usable_size(ptr); }
size_t malloc_usable_size(void* ptr) { return fex_malloc_usable_size(ptr); }
...
void fexfn_impl_libfex_malloc_fex_get_allocation_ptrs(AllocationPtrs* Ptrs) { *Ptrs = AllocationPointers; }
