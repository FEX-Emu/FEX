...
std::unordered_map<const clang::Type*, TypeInfo> ComputeDataLayout(const clang::ASTContext& context, const std::unordered_map<const clang::Type*, AnalysisAction::RepackedType>& types) {
    ...
    for (const auto& [type, type_repack_info] : types) {
        if (!type->isStructureType() || type_repack_info.is_opaque) {
            continue;
        }
...
TypeCompatibility DataLayoutCompareAction::GetTypeCompatibility(
    ...
    // TODO: This is also used by the assume_compatible_data_layout path now
    if (types.contains(type) && types.at(type).is_opaque) {
            ...
            if (host_member_type->isPointerType()) {
                ...
                if (types.contains(host_member_pointee_type) && types.at(host_member_pointee_type).is_opaque) {
                    ...
                    if (is_32bit) {
                        ...
                        member_compat.push_back(TypeCompatibility::None);
                    } else if (types.contains(host_member_pointee_type->getPointeeType().getTypePtr()) && types.at(host_member_pointee_type->getPointeeType().getTypePtr()).is_opaque) {
