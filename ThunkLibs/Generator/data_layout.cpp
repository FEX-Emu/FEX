      ...
      if (auto array_type = llvm::dyn_cast<clang::ConstantArrayType>(field->getType())) {
        ...
        if (llvm::isa<clang::ConstantArrayType>(field_type)) {
          throw std::runtime_error(
            "Unsupported multi-dimensional array member \"" + field->getNameAsString() + "\" in type \"" + clang::QualType {type, 0}.getAsString() + "\"");
        }
      }
    ...
    for (const auto& [type, info] : layout) {
      ...
      if (auto struct_info = info.get_if_struct()) {
        for (const auto& member : struct_info->members) {
          fprintf(
            stderr, "    Offset %lu-%lu: %s %s%s\n", member.offset_bits / 8, (member.offset_bits + member.size_bits - 1) / 8,
            member.type_name.c_str(), member.member_name.c_str(), member.array_size ? fmt::format("[{}]", member.array_size.value()).c_str() : "");
        }
      }
    }
  }
...
TypeCompatibility
DataLayoutCompareAction::GetTypeCompatibility(const clang::ASTContext& context, const clang::Type* type, const std::unordered_map<const clang::Type*, TypeInfo> host_abi, std::unordered_map<const clang::Type*, TypeCompatibility>& type_compat) {
      ...
      if (host_member_type->isPointerType()) {
        ...
        if (types.at(type).UsesCustomRepackFor(host_member_field)) {
          ...
          if (is_32bit) {
            ...
            types.contains(host_member_pointee_type->getPointeeType().getTypePtr()) &&
...
FuncPtrInfo DataLayoutCompareAction::LookupGuestFuncPtrInfo(const char* funcptr_id) { return guest_abi.thunked_funcptrs.at(funcptr_id); }
...
std::unique_ptr<clang::FrontendAction> DataLayoutCompareActionFactory::create() { return std::make_unique<DataLayoutCompareAction>(abi); }
...
std::unique_ptr<clang::FrontendAction> AnalyzeDataLayoutActionFactory::create() { return std::make_unique<AnalyzeDataLayoutAction>(*abi); }
