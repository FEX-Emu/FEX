      ...
      for (auto* decl : template_decl->specializations()) {
        ...
        if (auto emitted_function = llvm::dyn_cast<clang::FunctionDecl>(template_args[0].getAsDecl())) {
          ...
          for (int param_idx = retval_index; param_idx < (int)emitted_function->param_size(); ++param_idx) {
            ...
            auto param_loc =
              param_idx == retval_index ?
                emitted_function->getReturnTypeSourceRange().getBegin() :
            ...
            if (data.param_annotations[param_idx].is_passthrough && param_type->isPointerType() && param_type->getPointeeType()->isStructureType()) {
              continue;
            }
            ...
            if (param_type->isFunctionPointerType()) {
              ...
              param_type->isStructureType() &&
              !(types.contains(context.getCanonicalType(param_type.getTypePtr())) && LookupType(context, param_type.getTypePtr()).assumed_compatible)) {
              ...
              if (data.param_annotations[param_idx].assume_compatible) {
                ...
                types.contains(context.getCanonicalType(pointee_type.getTypePtr())) && LookupType(context, pointee_type.getTypePtr()).assumed_compatible) {
          ...
          thunked_api.push_back(ThunkedAPIFunction {
            (const FunctionParams&)data, data.function_name, data.return_type,
    ...
    for (auto next_type_it = types.begin(), type_it = next_type_it; type_it != types.end(); type_it = next_type_it) {
      ...
      for (auto* member : type->getAsStructureType()->getDecl()->fields()) {
        ...
        if (types.contains(member_type) && types.at(member_type).pointers_only) {
          if (member_type == context.getCanonicalType(member->getType().getTypePtr())) {
            throw std::runtime_error(fmt::format(
              "\"{}\" references opaque type \"{}\" via non-pointer member \"{}\"", clang::QualType {type, 0}.getAsString(),
        ...
        if (member_type->isUnionType() && !types.contains(member_type) && !type_repack_info.UsesCustomRepackFor(member)) {
          throw std::runtime_error(fmt::format(
            "\"{}\" has unannotated member \"{}\" of union type \"{}\"", clang::QualType {type, 0}.getAsString(), member->getNameAsString(),
