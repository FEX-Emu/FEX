...
struct compare_by_struct_dependency {
  ...
  bool
  operator()(const std::pair<const clang::Type*, GenerateThunkLibsAction::RepackedType>& a, const std::pair<const clang::Type*, GenerateThunkLibsAction::RepackedType>& b) const {
...
void GenerateThunkLibsAction::EmitLayoutWrappers(clang::ASTContext& context, std::ofstream& file, std::unordered_map<const clang::Type*, TypeCompatibility>& type_compat) {
  ...
  for (const auto& [type, type_repack_info] : types) {
    ...
    if (type_compat.at(type) == TypeCompatibility::Full) {
      ...
      for (auto& member : guest_abi.at(struct_name).get_if_struct()->members) {
        fmt::print(
          file, "    guest_layout<{}{}> {};\n", member.type_name, member.array_size ? fmt::format("[{}]", member.array_size.value()) : "",
          member.member_name);
      }
      fmt::print(file, "  }};\n");
    }
...
void GenerateThunkLibsAction::OnAnalysisComplete(clang::ASTContext& context) {
  ...
  if (!output_filenames.host.empty()) {
    ...
    for (auto& import : thunked_api) {
      ...
      file
        << "using fexldr_type_" << libname << "_" << function_name << " = auto "
    ...
    for (auto& thunk : thunks) {
      ...
      for (auto& [cb_idx, cb] : thunk.callbacks) {
        if (cb.is_stub) {
          ...
          file << "[[noreturn]] static " << cb.return_type.getAsString() << " " << cb_function_name << "(" << format_function_params(cb) << variadic_ellipsis << ") {\n";
      ...
      auto get_guest_type_name = [this](clang::QualType type) {
        if (type->isBuiltinType() && !type->isFloatingType()) {
          ...
          type->isPointerType() && type->getPointeeType()->isIntegerType() && !type->getPointeeType()->isEnumeralType() &&
          ...
          return fmt::format(
            "{}{}*", type->getPointeeType().isConstQualified() ? "const " : "", get_fixed_size_int_name(type->getPointeeType().getTypePtr(), size));
        } else {
        ...
        auto format_param = [&](std::size_t idx) {
          ...
          if (cb != thunk.callbacks.end() && cb->second.is_stub) {
            ...
            if (thunk.custom_host_impl) {
              ...
              return fmt::format(
                "(FinalizeHostTrampolineForGuestFunction({}), ({})(uint64_t {{ {}.data }}))", arg_name,
    ...
    for (auto& host_funcptr_entry : thunked_funcptrs) {
      ...
      fmt::print(
        file, "  {{(uint8_t*)\"\\x{:02x}\", (void(*)(void *))&GuestWrapperForHostFunction<{}({}){}{}>::Call<{}>}}, // {}\n",
...
bool GenerateThunkLibsActionFactory::runInvocation(std::shared_ptr<clang::CompilerInvocation> Invocation, clang::FileManager* Files, std::shared_ptr<clang::PCHContainerOperations> PCHContainerOps, clang::DiagnosticConsumer* DiagConsumer) {
