...
struct ThunkedFunction : FunctionParams {
    ...
    bool returns_from_host_trampoline = false;
...
class ASTVisitor : public clang::RecursiveASTVisitor<ASTVisitor> {
    ...
    Annotations GetAnnotations(clang::CXXRecordDecl* decl) {
        ...
        for (const auto& base : decl->bases()) {
            ...
            } else if (annotation == "fexgen::returns_from_host_trampoline") {
                ret.returns_from_host_trampoline = true;
...
public:
    ...
    bool VisitClassTemplateSpecializationDecl(clang::ClassTemplateSpecializationDecl* decl) try {
        ...
        if (return_type->isFunctionPointerType() && !(annotations.returns_guest_pointer || annotations.returns_from_host_trampoline)) {
        ...
        data.returns_from_host_trampoline = annotations.returns_from_host_trampoline;
        ...
        for (std::size_t param_idx = 0; param_idx < emitted_function->param_size(); ++param_idx) {
            ...
            if (param->getType()->isFunctionPointerType()) {
                ...
                callback.is_auto = annotations.callback_strategy == CallbackStrategy::Default;
        ...
        const bool has_nonstub_callbacks = std::any_of(data.callbacks.begin(), data.callbacks.end(),
                                                       [](auto& cb) { return !cb.second.is_stub && !cb.second.is_guest && !cb.second.is_auto; });
...
void GenerateThunkLibsAction::EndSourceFileAction() {
        ...
        for (auto& thunk : thunks) {
            ...
            for (auto& [cb_idx, cb] : thunk.callbacks) {
                if (cb.is_auto) {
                    ...
                    file << "MAKE_CALLBACK(" << libname << ", " << cb_function_name << ", ";
                    bool first = true;
    ...
    if (!output_filenames.function_packs.empty()) {
        ...
        for (auto& data : thunks) {
            ...
            for (std::size_t idx = 0; idx < data.param_types.size() - data.is_hostcall; ++idx) {
                ...
                if (cb != data.callbacks.end() && cb->second.is_auto) {
                    bool is_first_cb = (cb->first == data.callbacks.begin()->first);
                    auto cb_name = get_callback_name(function_name, cb->first, is_first_cb);
                    file << "  args.a_" << idx << " = HostTrampolineForGuestcall(fexcallback_" + libname + "_" + cb_name + ", &fexfn_unpack_" + libname + "_" + cb_name + ", a_" + std::to_string(idx) + ");\n";
                } else {
            ...
            if (!is_void) {
                if (data.returns_from_host_trampoline) {
                    file << "  return GuestcallTargetForHostTrampoline(args.rv);\n";
                } else {
                    file << "  return args.rv;\n";
                }
            }
    ...
    if (!output_filenames.function_unpacks.empty()) {
        ...
        for (auto& thunk : thunks) {
            ...
            auto GeneratePackedArgs = [&](const auto &function_name, const auto &thunk, const bool HasName) {
            ...
            for (auto& [cb_idx, cb] : thunk.callbacks) {
                if (cb.is_stub) {
                    ...
                    auto cb_function_name = get_callback_name(function_name, cb_idx, is_first_cb);
                    file << "[[noreturn]] static " << cb.return_type.getAsString() << " fexfn_pack_stubcall_"
                         << cb_function_name << "("
                    ...
                    file << "static " << cb.return_type.getAsString() << " fexfn_pack_guestcall_"
                         << cb_function_name << "("
                         << format_function_params(cb) << variadic_ellipsis << ") {\n";
                    file << "  CUSTOM_ABI_GUESTCALL;\n";
                    GeneratePackedArgs(cb_function_name, cb, false);
                    file << " argsrv;\n";
            ...
            GeneratePackedArgs(function_name, thunk, true);
            ...
            file << (is_void ? "  " : "  args->rv = ") << function_to_call << "(";
            {
                auto format_param = [&](std::size_t idx) {
                    ...
                    if (cb != thunk.callbacks.end() && cb->second.is_stub) {
                        ...
                        return "fexfn_pack_stubcall_" + get_callback_name(function_name, cb->first, is_first_cb);
