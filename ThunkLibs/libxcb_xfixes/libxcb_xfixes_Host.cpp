...
void fexfn_unpack_libxcb_xfixes_FEX_xcb_xfixes_init_extension(void *argsv);

void fexfn_unpack_libxcb_xfixes_FEX_usable_size(void *argsv){
  struct arg_t {void* a_0;size_t rv;};
  auto args = (arg_t*)argsv;
  args->rv = malloc_usable_size(args->a_0);
}

void fexfn_unpack_libxcb_xfixes_FEX_free_on_host(void *argsv){
...
void fexfn_unpack_libxcb_xfixes_FEX_xcb_xfixes_init_extension(void *argsv) {
  ...
  auto args = (arg_t*)argsv;
  xcb_extension_t *ext{};
  if (strcmp(args->a_1->name, "XFIXES") == 0) {
