...
void x64SyscallHandler::Strace(FEXCore::HLE::SyscallArguments *Args, uint64_t Ret) {
  ...
  case SYSCALL_UNAME:
    ...
    break;
  case SYSCALL_SEMGET:
    LogMan::Msg::D("semget(0x%lx, %d, 0x%lx) = %ld",
      Args->Argument[1],
      Args->Argument[2],
      Args->Argument[3],
      Ret);
  ...
  case SYSCALL_SEMOP:
    ...
    break;
  case SYSCALL_SEMCTL:
    LogMan::Msg::D("semctl(%ld, %ld, %ld, %p) = %ld",
      Args->Argument[1],
      Args->Argument[2],
      Args->Argument[3],
      Args->Argument[4],
  ...
  case SYSCALL_FCNTL:
    ...
    break;
  case SYSCALL_FLOCK:
    LogMan::Msg::D("flock(%ld, %ld) = %ld", Args->Argument[1], Args->Argument[2], Ret);
    break;
  case SYSCALL_FSYNC:
    LogMan::Msg::D("fsync(%ld) = %ld", Args->Argument[1], Ret);
    break;
  case SYSCALL_FDATASYNC:
    LogMan::Msg::D("fdatasync(%ld) = %ld", Args->Argument[1], Ret);
    break;
  ...
  case SYSCALL_CHMOD:
    ...
    break;
  case SYSCALL_FCHMOD:
    LogMan::Msg::D("fchmod(%d, 0x%lx) = %ld", Args->Argument[1], Args->Argument[2], Ret);
    break;
  case SYSCALL_EXIT:
    LogMan::Msg::D("exit(0x%lx)", Args->Argument[1]);
    break;
  case SYSCALL_WAIT4:
    LogMan::Msg::D("wait4(0x%lx, 0x%lx, %ld, 0x%lx) = %ld",
      Args->Argument[1],
      Args->Argument[2],
      Args->Argument[3],
      Args->Argument[4],
  ...
  case SYSCALL_EPOLL_CREATE:
    LogMan::Msg::D("epoll_create(%ld) = %ld", Args->Argument[1], Ret);
    break;
  ...
  case SYSCALL_SET_TID_ADDRESS:
    ...
    break;
  case SYSCALL_SEMTIMEDOP:
    LogMan::Msg::D("semtimedop(%d, %p, %d, %p) = %ld",
      Args->Argument[1],
      Args->Argument[2],
      Args->Argument[3],
      Args->Argument[4],
  ...
  case SYSCALL_FALLOCATE:
    LogMan::Msg::D("fallocate(%lx, %lx, %p, %p) = %ld",
      Args->Argument[1],
      Args->Argument[2],
      Args->Argument[3],
      Args->Argument[4],
...
void x64SyscallHandler::RegisterSyscallHandlers() {
  ...
  const std::vector<std::tuple<uint16_t, void*, uint8_t>> Syscalls = {
    ...
    {SYSCALL_RT_SIGACTION,           cvt(&FEXCore::HLE::RT_Sigaction),           3},
    {SYSCALL_RT_SIGPROCMASK,         cvt(&FEXCore::HLE::RT_Sigprocmask),         3},
    ...
    {SYSCALL_KILL,                   cvt(&FEXCore::HLE::Kill),                   2},
    {SYSCALL_UNAME,                  cvt(&FEXCore::HLE::Uname),                  1},
    {SYSCALL_SEMGET,                 cvt(&FEXCore::HLE::Semget),                 3},
    {SYSCALL_SEMOP,                  cvt(&FEXCore::HLE::Semop),                  3},
    {SYSCALL_SEMCTL,                 cvt(&FEXCore::HLE::Semctl),                 4},
    {SYSCALL_SHMDT,                  cvt(&FEXCore::HLE::Shmdt),                  1},
    {SYSCALL_FCNTL,                  cvt(&FEXCore::HLE::Fcntl),                  3},
    {SYSCALL_FLOCK,                  cvt(&FEXCore::HLE::Flock),                  2},
    ...
    {SYSCALL_SIGALTSTACK,            cvt(&FEXCore::HLE::Sigaltstack),            2},
    ...
    {SYSCALL_SCHED_SETPARAM,         cvt(&FEXCore::HLE::Sched_Setparam),         2},
    {SYSCALL_SCHED_GETPARAM,         cvt(&FEXCore::HLE::Sched_Getparam),         2},
    {SYSCALL_SCHED_SETSCHEDULER,     cvt(&FEXCore::HLE::Sched_Setscheduler),     3},
    {SYSCALL_SCHED_GETSCHEDULER,     cvt(&FEXCore::HLE::Sched_Getscheduler),     1},
    {SYSCALL_SCHED_GET_PRIORITY_MAX, cvt(&FEXCore::HLE::Sched_Get_priority_max), 1},
    {SYSCALL_SCHED_GET_PRIORITY_MIN, cvt(&FEXCore::HLE::Sched_Get_priority_min), 1},
    {SYSCALL_SCHED_RR_GET_INTERVAL,  cvt(&FEXCore::HLE::Sched_rr_get_interval),  2},
    ...
    {SYSCALL_EPOLL_CREATE,           cvt(&FEXCore::HLE::EPoll_Create),           1},
    ...
    {SYSCALL_FALLOCATE,              cvt(&FEXCore::HLE::Fallocate),              4},
