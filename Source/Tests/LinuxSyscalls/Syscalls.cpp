...
uint32_t SyscallHandler::CalculateGuestKernelVersion() {
  // We currently only emulate a kernel between the ranges of Kernel 5.0.0 and 5.16.0
  return std::max(KernelVersion(5, 0), std::min(KernelVersion(5, 16), GetHostKernelVersion()));
}
