...
namespace FEX::HLE {
  void RegisterEpoll() {
    ...
    REGISTER_SYSCALL_IMPL_PASS_FLAGS(epoll_create, SYSCALL_FLAG_OPTIMIZETHROUGH | SYSCALL_FLAG_NOSYNCSTATEONENTRY,
      [](FEXCore::Core::CpuStateFrame *Frame, int size) -> uint64_t {
      uint64_t Result = epoll_create(size);
      SYSCALL_ERRNO();
    });

    REGISTER_SYSCALL_IMPL_PASS_FLAGS(epoll_create1, SYSCALL_FLAG_OPTIMIZETHROUGH | SYSCALL_FLAG_NOSYNCSTATEONENTRY,
