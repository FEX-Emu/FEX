...
#define REGISTER_SYSCALL_IMPL_X64(name, lambda) \
  struct impl_##name { \
    impl_##name() \
    { \
      FEX::HLE::x64::RegisterSyscall(x64::SYSCALL_x64_##name, ~0, FEXCore::IR::SYSCALL_FLAG_DEFAULT, #name, lambda); \
    } } impl_##name
...
#define REGISTER_SYSCALL_IMPL_X64_PASS(name, lambda) \
  struct impl_##name { \
    impl_##name() \
    { \
      FEX::HLE::x64::RegisterSyscall(x64::SYSCALL_x64_##name, SYSCALL_DEF(name), FEXCore::IR::SYSCALL_FLAG_DEFAULT, #name, lambda); \
    } } impl_##name
