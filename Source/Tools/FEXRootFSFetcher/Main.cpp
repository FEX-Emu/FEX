...
std::optional<std::vector<FileTargets>> GetRootFSLinks() {
  ...
  for (const json_t* RootItem = json_getChild(RootList); RootItem != nullptr; RootItem = json_getSibling(RootItem)) {
    ...
    if ((Target.Type == FileTargets::FileType::TYPE_SQUASHFS && SupportsSquashFS) || (Target.Type == FileTargets::FileType::TYPE_EROFS && SupportsEroFS)) {
...
bool AskForConfirmation(const fextl::string& Question) { return ArgOptions::AssumeYes || ExecWithQuestion(Question); }
...
void ExecWithInfo(const fextl::string& Text) { std::cout << Text << std::endl; }
...
bool AskForConfirmation(const fextl::string& Question) { return _AskForConfirmation(Question); }

void ExecWithInfo(const fextl::string& Text) { _ExecWithInfo(Text); }
...
bool ValidateCheckExists(const WebFileFetcher::FileTargets& Target) { return _ValidateCheckExists(Target); }

bool ValidateDownloadSelection(const WebFileFetcher::FileTargets& Target) { return _ValidateDownloadSelection(Target); }
} // namespace
