...
int main(int argc, char **argv) {
  ...
  for (const auto &Entry : std::filesystem::directory_iterator("/proc/")) {
    ...
    if (!std::filesystem::exists(CMDLinePath)) continue;
    ...
    std::ostringstream CMDLineData;
    {
      ...
      if (CMDLineData.str().empty()) continue;
    }
  ...
  for (const auto &pid : PIDs) {
    if (pid.State == 'Z' && Config::SkipZombie) continue;
    if (Config::OmitPids.contains(pid.pid)) continue;
    ...
    auto IsFEX = [](auto &Path) {
      if (Path.ends_with("FEXInterpreter")) return true;
      if (Path.ends_with("FEXLoader")) return true;
