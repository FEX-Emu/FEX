...
static bool OpenSavedPopup{};

static const char SavedPopupName[] = "#Saved";
...
static std::chrono::high_resolution_clock::time_point SavedTimerStart{};
...
void SaveFile(std::string Filename) {
  if (!ConfigOpen) {
	...
	ConfigFilename = Filename;
	OpenSavedPopup = true;
	SavedTimerStart = std::chrono::high_resolution_clock::now();
...
void DrawUI(GLContext::Context *Context) {
  ...
  if (ConfigOpen) {
		...
		if (OpenSavedPopup) {
			ImGui::OpenPopup(SavedPopupName);
		...
		if (ImGui::BeginPopup(SavedPopupName)) {
			ImGui::Text("Config Saved to: '%s'", ConfigFilename.c_str());
			if ((std::chrono::high_resolution_clock::now() - SavedTimerStart) >= std::chrono::seconds(2)) {
				ImGui::CloseCurrentPopup();
			}
			ImGui::EndPopup();
		}
