...
auto selectHandler = [](FEXCore::Core::CpuStateFrame* Frame, int nfds, fd_set32* readfds, fd_set32* writefds, fd_set32* exceptfds, struct timeval32* timeout) -> uint64_t {
  ...
  uint64_t Result = ::select(nfds, readfds ? &Host_readfds : nullptr, writefds ? &Host_writefds : nullptr, exceptfds ? &Host_exceptfds : nullptr, timeout ? &tp64 : nullptr);
  if (readfds) {
...
void RegisterFD(FEX::HLE::SyscallHandler* Handler) {
  ...
  REGISTER_SYSCALL_IMPL_X32(preadv, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high) -> uint64_t {
    fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
  ...
  REGISTER_SYSCALL_IMPL_X32(pwritev, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high) -> uint64_t {
    fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
  ...
  REGISTER_SYSCALL_IMPL_X32(preadv2, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high, int flags) -> uint64_t {
    fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
  ...
  REGISTER_SYSCALL_IMPL_X32(pwritev2, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high, int flags) -> uint64_t {
    fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
  ...
  REGISTER_SYSCALL_IMPL_X32(pselect6, [](FEXCore::Core::CpuStateFrame* Frame, int nfds, fd_set32* readfds, fd_set32* writefds, fd_set32* exceptfds, timespec32* timeout, compat_ptr<sigset_argpack32> sigmaskpack) -> uint64_t {
    ...
    uint64_t Result = ::pselect(nfds, readfds ? &Host_readfds : nullptr, writefds ? &Host_writefds : nullptr, exceptfds ? &Host_exceptfds : nullptr, timeout ? &tp64 : nullptr, &HostSet);
  ...
  REGISTER_SYSCALL_IMPL_X32(pselect6_time64, [](FEXCore::Core::CpuStateFrame* Frame, int nfds, fd_set32* readfds, fd_set32* writefds, fd_set32* exceptfds, struct timespec* timeout, compat_ptr<sigset_argpack32> sigmaskpack) -> uint64_t {
    ...
    uint64_t Result = ::pselect(nfds, readfds ? &Host_readfds : nullptr, writefds ? &Host_writefds : nullptr, exceptfds ? &Host_exceptfds : nullptr, timeout, &HostSet);
  ...
  REGISTER_SYSCALL_IMPL_X32(sync_file_range, [](FEXCore::Core::CpuStateFrame* Frame, int fd, uint32_t offset_low, uint32_t offset_high, uint32_t len_low, uint32_t len_high, unsigned int flags) -> uint64_t {
    // Flags don't need remapped
    uint64_t Offset = offset_high;
    Offset <<= 32;
    Offset |= offset_low;
  ...
  REGISTER_SYSCALL_IMPL_X32(fallocate, [](FEXCore::Core::CpuStateFrame* Frame, int fd, int mode, uint32_t offset_low, uint32_t offset_high, uint32_t len_low, uint32_t len_high) -> uint64_t {
    uint64_t Offset = offset_high;
    Offset <<= 32;
    Offset |= offset_low;
