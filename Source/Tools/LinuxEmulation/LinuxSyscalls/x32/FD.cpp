...
auto ppoll(FEXCore::Core::CpuStateFrame* Frame, struct pollfd* fds, nfds_t nfds, timespec32* timeout_ts, const uint64_t* sigmask,
           size_t sigsetsize) -> uint64_t {
...
auto _llseek(FEXCore::Core::CpuStateFrame* Frame, uint32_t fd, uint32_t offset_high, uint32_t offset_low, loff_t* result, uint32_t whence)
  -> uint64_t {
...
auto readv(FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, int iovcnt) -> uint64_t {
...
auto writev(FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, int iovcnt) -> uint64_t {
  FaultSafeUserMemAccess::VerifyIsReadable(iov, sizeof(struct iovec32) * SanitizeIOCount(iovcnt));
  fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
...
auto chown32(FEXCore::Core::CpuStateFrame* Frame, const char* pathname, uid_t owner, gid_t group) -> uint64_t {
  uint64_t Result = ::chown(pathname, owner, group);
  SYSCALL_ERRNO();
}

auto lchown32(FEXCore::Core::CpuStateFrame* Frame, const char* pathname, uid_t owner, gid_t group) -> uint64_t {
  uint64_t Result = ::lchown(pathname, owner, group);
  SYSCALL_ERRNO();
}

auto oldstat(FEXCore::Core::CpuStateFrame* Frame, const char* pathname, oldstat32* buf) -> uint64_t {
...
auto oldfstat(FEXCore::Core::CpuStateFrame* Frame, int fd, oldstat32* buf) -> uint64_t {
  struct stat host_stat;
...
auto oldlstat(FEXCore::Core::CpuStateFrame* Frame, const char* path, oldstat32* buf) -> uint64_t {
  struct stat host_stat;
...
auto stat(FEXCore::Core::CpuStateFrame* Frame, const char* pathname, stat32* buf) -> uint64_t {
  struct stat host_stat;
  uint64_t Result = FEX::HLE::_SyscallHandler->FM.Stat(pathname, &host_stat);
  if (Result != -1) {
    FaultSafeUserMemAccess::VerifyIsWritable(buf, sizeof(*buf));
...
auto fstat(FEXCore::Core::CpuStateFrame* Frame, int fd, stat32* buf) -> uint64_t {
  struct stat host_stat;
  uint64_t Result = ::fstat(fd, &host_stat);
  if (Result != -1) {
    FaultSafeUserMemAccess::VerifyIsWritable(buf, sizeof(*buf));
...
auto lstat(FEXCore::Core::CpuStateFrame* Frame, const char* path, stat32* buf) -> uint64_t {
  struct stat host_stat;
  uint64_t Result = FEX::HLE::_SyscallHandler->FM.Lstat(path, &host_stat);
  if (Result != -1) {
    FaultSafeUserMemAccess::VerifyIsWritable(buf, sizeof(*buf));
...
auto stat64(FEXCore::Core::CpuStateFrame* Frame, const char* pathname, stat64_32* buf) -> uint64_t {
  struct stat host_stat;
  uint64_t Result = FEX::HLE::_SyscallHandler->FM.Stat(pathname, &host_stat);
  if (Result != -1) {
    FaultSafeUserMemAccess::VerifyIsWritable(buf, sizeof(*buf));
...
auto lstat64(FEXCore::Core::CpuStateFrame* Frame, const char* path, stat64_32* buf) -> uint64_t {
  struct stat host_stat;
  uint64_t Result = FEX::HLE::_SyscallHandler->FM.Lstat(path, &host_stat);
...
auto fstat64(FEXCore::Core::CpuStateFrame* Frame, int fd, stat64_32* buf) -> uint64_t {
...
auto statfs(FEXCore::Core::CpuStateFrame* Frame, const char* path, statfs32_32* buf) -> uint64_t {
...
auto fstatfs(FEXCore::Core::CpuStateFrame* Frame, int fd, statfs32_32* buf) -> uint64_t {
  struct statfs host_stat;
...
auto fstatfs64(FEXCore::Core::CpuStateFrame* Frame, int fd, size_t sz, struct statfs64_32* buf) -> uint64_t {
...
auto statfs64(FEXCore::Core::CpuStateFrame* Frame, const char* path, size_t sz, struct statfs64_32* buf) -> uint64_t {
  LOGMAN_THROW_A_FMT(sz == sizeof(struct statfs64_32), "This needs to match");
...
auto dup(FEXCore::Core::CpuStateFrame* Frame, int oldfd) -> uint64_t {
...
auto preadv(FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high)
  -> uint64_t {
  FaultSafeUserMemAccess::VerifyIsReadable(iov, sizeof(struct iovec32) * SanitizeIOCount(iovcnt));
  fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
...
auto pwritev(FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high)
  -> uint64_t {
  FaultSafeUserMemAccess::VerifyIsReadable(iov, sizeof(struct iovec32) * SanitizeIOCount(iovcnt));
  fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
...
auto process_vm_readv(FEXCore::Core::CpuStateFrame* Frame, pid_t pid, const struct iovec32* local_iov, unsigned long liovcnt,
...
auto process_vm_writev(FEXCore::Core::CpuStateFrame* Frame, pid_t pid, const struct iovec32* local_iov, unsigned long liovcnt,
                       const struct iovec32* remote_iov, unsigned long riovcnt, unsigned long flags) -> uint64_t {
  FaultSafeUserMemAccess::VerifyIsReadable(local_iov, sizeof(struct iovec32) * SanitizeIOCount(liovcnt));
  FaultSafeUserMemAccess::VerifyIsReadable(remote_iov, sizeof(struct iovec32) * SanitizeIOCount(riovcnt));
...
auto preadv2(FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high,
             int flags) -> uint64_t {
  FaultSafeUserMemAccess::VerifyIsReadable(iov, sizeof(struct iovec32) * SanitizeIOCount(iovcnt));
  fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
...
auto pwritev2(FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, uint32_t iovcnt, uint32_t pos_low, uint32_t pos_high,
              int flags) -> uint64_t {
  FaultSafeUserMemAccess::VerifyIsReadable(iov, sizeof(struct iovec32) * SanitizeIOCount(iovcnt));
  fextl::vector<iovec> Host_iovec(iov, iov + SanitizeIOCount(iovcnt));
...
auto fstatat_64(FEXCore::Core::CpuStateFrame* Frame, int dirfd, const char* pathname, stat64_32* buf, int flag) -> uint64_t {
  struct stat64 host_stat;
...
auto select(FEXCore::Core::CpuStateFrame* Frame, compat_select_args* arg) -> uint64_t {
  return selectHandler(Frame, arg->nfds, arg->readfds, arg->writefds, arg->exceptfds, arg->timeout);
}

auto pselect6(FEXCore::Core::CpuStateFrame* Frame, int nfds, fd_set32* readfds, fd_set32* writefds, fd_set32* exceptfds,
...
auto fadvise64(FEXCore::Core::CpuStateFrame* Frame, int32_t fd, uint32_t offset_low, uint32_t offset_high, uint32_t len, int advice) -> uint64_t {
  uint64_t Offset = offset_high;
  Offset <<= 32;
  Offset |= offset_low;
...
auto fadvise64_64(FEXCore::Core::CpuStateFrame* Frame, int32_t fd, uint32_t offset_low, uint32_t offset_high, uint32_t len_low,
...
auto timerfd_settime(FEXCore::Core::CpuStateFrame* Frame, int fd, int flags, const FEX::HLE::x32::old_itimerspec32* new_value,
...
auto timerfd_gettime(FEXCore::Core::CpuStateFrame* Frame, int fd, FEX::HLE::x32::old_itimerspec32* curr_value) -> uint64_t {
...
auto pselect6_time64(FEXCore::Core::CpuStateFrame* Frame, int nfds, fd_set32* readfds, fd_set32* writefds, fd_set32* exceptfds,
...
auto sendfile(FEXCore::Core::CpuStateFrame* Frame, int out_fd, int in_fd, compat_off_t* offset, size_t count) -> uint64_t {
  off_t Local {};
  off_t* Local_p {};
  if (offset) {
    Local_p = &Local;
...
auto pread_64(FEXCore::Core::CpuStateFrame* Frame, int fd, void* buf, uint32_t count, uint32_t offset_low, uint32_t offset_high) -> uint64_t {
  uint64_t Offset = offset_high;
  Offset <<= 32;
  Offset |= offset_low;
...
auto pwrite_64(FEXCore::Core::CpuStateFrame* Frame, int fd, void* buf, uint32_t count, uint32_t offset_low, uint32_t offset_high) -> uint64_t {
  uint64_t Offset = offset_high;
  Offset <<= 32;
  Offset |= offset_low;
...
auto readahead(FEXCore::Core::CpuStateFrame* Frame, int fd, uint32_t offset_low, uint64_t offset_high, size_t count) -> uint64_t {
  uint64_t Offset = offset_high;
  Offset <<= 32;
  Offset |= offset_low;
...
auto sync_file_range(FEXCore::Core::CpuStateFrame* Frame, int fd, uint32_t offset_low, uint32_t offset_high, uint32_t len_low,
...
auto fallocate(FEXCore::Core::CpuStateFrame* Frame, int fd, int mode, uint32_t offset_low, uint32_t offset_high, uint32_t len_low,
...
auto vmsplice(FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec32* iov, unsigned long nr_segs, unsigned int flags) -> uint64_t {
...
void RegisterFD(FEX::HLE::SyscallHandler* Handler) {
  REGISTER_SYSCALL_IMPL_X32(ppoll, ppoll);
  REGISTER_SYSCALL_IMPL_X32(_llseek, _llseek);
  REGISTER_SYSCALL_IMPL_X32(readv, readv);
  REGISTER_SYSCALL_IMPL_X32(writev, writev);
  REGISTER_SYSCALL_IMPL_X32(chown32, chown32);
  REGISTER_SYSCALL_IMPL_X32(lchown32, lchown32);
  REGISTER_SYSCALL_IMPL_X32(oldstat, oldstat);
  REGISTER_SYSCALL_IMPL_X32(oldfstat, oldfstat);
  REGISTER_SYSCALL_IMPL_X32(oldlstat, oldlstat);
  REGISTER_SYSCALL_IMPL_X32(stat, stat);
  REGISTER_SYSCALL_IMPL_X32(fstat, fstat);
  REGISTER_SYSCALL_IMPL_X32(lstat, lstat);
  REGISTER_SYSCALL_IMPL_X32(stat64, stat64);
  REGISTER_SYSCALL_IMPL_X32(lstat64, lstat64);
  REGISTER_SYSCALL_IMPL_X32(fstat64, fstat64);
  REGISTER_SYSCALL_IMPL_X32(statfs, statfs);
  REGISTER_SYSCALL_IMPL_X32(fstatfs, fstatfs);
  REGISTER_SYSCALL_IMPL_X32(fstatfs64, fstatfs64);
  REGISTER_SYSCALL_IMPL_X32(statfs64, statfs64);
  ...
  REGISTER_SYSCALL_IMPL_X32(dup, dup);
  REGISTER_SYSCALL_IMPL_X32(dup2, dup2);
  REGISTER_SYSCALL_IMPL_X32(preadv, preadv);
  REGISTER_SYSCALL_IMPL_X32(pwritev, pwritev);
  REGISTER_SYSCALL_IMPL_X32(process_vm_readv, process_vm_readv);
  REGISTER_SYSCALL_IMPL_X32(process_vm_writev, process_vm_writev);
  REGISTER_SYSCALL_IMPL_X32(preadv2, preadv2);
  REGISTER_SYSCALL_IMPL_X32(pwritev2, pwritev2);
  REGISTER_SYSCALL_IMPL_X32(fstatat_64, fstatat_64);
  REGISTER_SYSCALL_IMPL_X32(ioctl, ioctl32);
  REGISTER_SYSCALL_IMPL_X32(getdents, getdents);
  REGISTER_SYSCALL_IMPL_X32(getdents64, getdents64);
  REGISTER_SYSCALL_IMPL_X32(select, select);
  REGISTER_SYSCALL_IMPL_X32(_newselect, selectHandler);
  REGISTER_SYSCALL_IMPL_X32(pselect6, pselect6);
  REGISTER_SYSCALL_IMPL_X32(fadvise64, fadvise64);
  REGISTER_SYSCALL_IMPL_X32(fadvise64_64, fadvise64_64);
  REGISTER_SYSCALL_IMPL_X32(timerfd_settime, timerfd_settime);
  REGISTER_SYSCALL_IMPL_X32(timerfd_gettime, timerfd_gettime);
  REGISTER_SYSCALL_IMPL_X32(pselect6_time64, pselect6_time64);
  REGISTER_SYSCALL_IMPL_X32(sendfile, sendfile);
  REGISTER_SYSCALL_IMPL_X32(pread_64, pread_64);
  REGISTER_SYSCALL_IMPL_X32(pwrite_64, pwrite_64);
  REGISTER_SYSCALL_IMPL_X32(readahead, readahead);
  REGISTER_SYSCALL_IMPL_X32(sync_file_range, sync_file_range);
  REGISTER_SYSCALL_IMPL_X32(fallocate, fallocate);
  REGISTER_SYSCALL_IMPL_X32(vmsplice, vmsplice);
}
} // namespace FEX::HLE::x32
