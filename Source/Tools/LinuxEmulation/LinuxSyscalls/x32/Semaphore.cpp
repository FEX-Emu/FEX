...
uint64_t _ipc(FEXCore::Core::CpuStateFrame* Frame, uint32_t call, uint32_t first, uint32_t second, uint32_t third, uint32_t ptr, uint32_t fifth) {
  ...
  case OP_SHMAT: {
    ...
    Result = static_cast<FEX::HLE::x32::x32SyscallHandler*>(FEX::HLE::_SyscallHandler)->GetAllocator()->Shmat(first, reinterpret_cast<const void*>(ptr), second, reinterpret_cast<uint32_t*>(third));
  ...
  case OP_SHMCTL: {
    ...
    case SHM_LOCK: Result = ::syscall(SYSCALL_DEF(_shmctl), shmid, cmd, nullptr); break;
    case SHM_UNLOCK: Result = ::syscall(SYSCALL_DEF(_shmctl), shmid, cmd, nullptr); break;
    case IPC_RMID: Result = ::syscall(SYSCALL_DEF(_shmctl), shmid, cmd, nullptr); break;
