...
#undef COPY_REG
    ...
    if (IsAVXEnabled) {
      CTX->SetXMMRegistersFromState(Thread, fpstate->_xmm, xstate->ymmh.ymmh_space, FEXCore::Context::Context::XMMRecoveryMode::YMM);
    } else {
      CTX->SetXMMRegistersFromState(Thread, fpstate->_xmm, nullptr, FEXCore::Context::Context::XMMRecoveryMode::XMM);
    }
...
#undef COPY_REG
  ...
  if (IsAVXEnabled) {
    CTX->ReconstructXMMRegisters(Thread, fpstate->_xmm, xstate->ymmh.ymmh_space, FEXCore::Context::Context::XMMRecoveryMode::YMM);
  } else {
    CTX->ReconstructXMMRegisters(Thread, fpstate->_xmm, nullptr, FEXCore::Context::Context::XMMRecoveryMode::XMM);
  }
