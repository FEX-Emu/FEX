...
void SyscallHandler::VMATracking::SetUnsafe(FEXCore::Context::Context* CTX, MappedResource* MappedResource, uintptr_t Base, uintptr_t Offset, uintptr_t Length, VMAFlags Flags, VMAProt Prot) {
  ClearUnsafe(CTX, Base, Length, MappedResource);

  auto [Iter, Inserted] = VMAs.emplace(Base, VMAEntry {MappedResource, nullptr, MappedResource ? MappedResource->FirstVMA : nullptr, Base, Offset, Length, Flags, Prot});
    ...
    if (MapTop <= Base) {
      ...
      if (HasTrailingPart) {
        ...
        auto [Iter, Inserted] = VMAs.emplace(
          Top,
          VMAEntry {
            Current->Resource, ReplaceAndErase ? Current->ResourcePrevVMA : Current, Current->ResourceNextVMA, Top, NewOffset, NewLength,
            Current->Flags, Current->Prot});
  ...
  while (MappingIter != VMAs.begin()) {
    ...
    if (MapTop <= Base) {
      ...
      if (HasFirstPart) {
        ...
        auto [Iter, Inserted] = VMAs.emplace(Base, VMAEntry {Current->Resource, Current, Current->ResourceNextVMA, Base, NewOffset, NewLength, MapFlags, NewProt});
        LOGMAN_THROW_A_FMT(Inserted == true, "VMA tracking error");
      ...
      if (HasTrailingPart) {
        ...
        auto [Iter, Inserted] = VMAs.emplace(Top, VMAEntry {Current->Resource, Current, Current->ResourceNextVMA, Top, NewOffset, NewLength, MapFlags, MapProt});
        LOGMAN_THROW_A_FMT(Inserted == true, "VMA tracking error");
...
uintptr_t SyscallHandler::VMATracking::ClearShmUnsafe(FEXCore::Context::Context* CTX, uintptr_t Base) {
  ...
  for (; Entry != VMAs.end(); ++Entry) {
    ...
    if (Entry->second.Base - Base == Entry->second.Offset && Entry->second.Resource && Entry->second.Resource->Iterator->first.dev == SpecialDev::SHM) {
      break;
    }
  }
