...
void* MemAllocator32Bit::Mmap(void* addr, size_t length, int prot, int flags, int fd, off_t offset) {
  ...
  if (fd != -1 && offset & ~FHU::FEX_PAGE_MASK) {
    return reinterpret_cast<void*>(-EINVAL);
  }
...
restart: {
  ...
  }
  {
    ...
    void* MappedPtr = ::mmap(reinterpret_cast<void*>(LowerPage << FHU::FEX_PAGE_SHIFT), length, prot, flags | FEX_MAP_FIXED_NOREPLACE, fd, offset);

    if (MappedPtr == MAP_FAILED && errno != EEXIST) {
      return reinterpret_cast<void*>(-errno);
    } else if (MappedPtr == MAP_FAILED || MappedPtr >= reinterpret_cast<void*>(TOP_KEY << FHU::FEX_PAGE_SHIFT)) {
      ...
      if (MappedPtr != MAP_FAILED && MappedPtr >= reinterpret_cast<void*>(TOP_KEY << FHU::FEX_PAGE_SHIFT)) {
  ...
  if (Addr == 0) {
    ...
    void* MappedPtr = ::mmap(reinterpret_cast<void*>(PageAddr << FHU::FEX_PAGE_SHIFT), PagesLength << FHU::FEX_PAGE_SHIFT, prot, flags, fd, offset);

    if (MappedPtr >= reinterpret_cast<void*>(TOP_KEY << FHU::FEX_PAGE_SHIFT) && (flags & FEX_MAP_FIXED_NOREPLACE)) {
...
restart: {
  ...
  {
    // Try and map the range
    void* MappedPtr = ::shmat(shmid, reinterpret_cast<const void*>(LowerPage << FHU::FEX_PAGE_SHIFT), shmflg);
...
fextl::unique_ptr<FEX::HLE::MemAllocator> Create32BitAllocator() { return fextl::make_unique<MemAllocator32Bit>(); }

fextl::unique_ptr<FEX::HLE::MemAllocator> CreatePassthroughAllocator() { return fextl::make_unique<MemAllocatorPassThrough>(); }
