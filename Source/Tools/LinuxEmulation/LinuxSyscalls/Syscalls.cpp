...
uint64_t GetDentsEmulation(int fd, T* dirp, uint32_t count) {
  ...
  if (Result != -1) {
    ...
    while (TmpOffset < Result) {
      ...
      size_t NewRecLen = FEXCore::AlignUp(
        Tmp->d_reclen - (sizeof(std::remove_reference<decltype(*Tmp)>::type) - sizeof(*Outgoing)), alignof(decltype(Tmp->d_ino)));
...
uint64_t ExecveHandler(const char* pathname, char* const* argv, char* const* envp, ExecveAtArgs Args) {
  ...
  uint64_t Result {};
  if (FEX::HLE::_SyscallHandler->IsInterpreterInstalled() && FEX::HLE::_SyscallHandler->IsInterpreter() && (Type == ELFLoader::ELFContainer::ELFType::TYPE_X86_32 || Type == ELFLoader::ELFContainer::ELFType::TYPE_X86_64)) {
...
static bool AnyFlagsSet(uint64_t Flags, uint64_t Mask) { return (Flags & Mask) != 0; }

static bool AllFlagsSet(uint64_t Flags, uint64_t Mask) { return (Flags & Mask) == Mask; }
...
[[noreturn]] static void Clone3HandlerRet() {
...
uint64_t CloneHandler(FEXCore::Core::CpuStateFrame* Frame, FEX::HLE::clone3_args* args) {
  ...
  auto HasUnhandledFlags = [](FEX::HLE::clone3_args* args) -> bool {
    constexpr uint64_t UNHANDLED_FLAGS =
      CLONE_NEWNS |
      // CLONE_UNTRACED |
      CLONE_NEWCGROUP | CLONE_NEWUTS | CLONE_NEWUTS | CLONE_NEWIPC | CLONE_NEWUSER | CLONE_NEWPID | CLONE_NEWNET | CLONE_IO | CLONE_CLEAR_SIGHAND | CLONE_INTO_CGROUP;
  ...
  if (!(flags & CLONE_THREAD)) {
    ...
    return FEX::HLE::ForkGuest(
      Thread, Frame, flags, reinterpret_cast<void*>(args->args.stack), args->args.stack_size, reinterpret_cast<pid_t*>(args->args.parent_tid),
      reinterpret_cast<pid_t*>(args->args.child_tid), reinterpret_cast<void*>(args->args.tls));
  } else {
...
SyscallHandler::~SyscallHandler() { FEXCore::Allocator::munmap(reinterpret_cast<void*>(DataSpace), DataSpaceMaxSize); }
...
uint64_t SyscallHandler::HandleSyscall(FEXCore::Core::CpuStateFrame* Frame, FEXCore::HLE::SyscallArguments* Args) {
  ...
  case 6:
    Result = std::invoke(
      Def.Ptr6, Frame, Args->Argument[1], Args->Argument[2], Args->Argument[3], Args->Argument[4], Args->Argument[5], Args->Argument[6]);
    break;
...
void SyscallHandler::Strace(FEXCore::HLE::SyscallArguments* Args, uint64_t Ret) {
  ...
  case 6:
    LogMan::Msg::D(
      Def.StraceFmt.c_str(), Args->Argument[1], Args->Argument[2], Args->Argument[3], Args->Argument[4], Args->Argument[5], Args->Argument[6], Ret);
    break;
  default: break;
  }
}
...
uint64_t UnimplementedSyscallSafe(FEXCore::Core::CpuStateFrame* Frame, uint64_t SyscallNumber) { return -ENOSYS; }
...
DoGenerate:
    ...
    std::sort(rv->SortedLineMappings.begin(), rv->SortedLineMappings.end(), [](const auto& lhs, const auto& rhs) {
      return lhs.FileGuestEnd <= rhs.FileGuestBegin;
    });

    std::sort(rv->SortedSymbolMappings.begin(), rv->SortedSymbolMappings.end(), [](const auto& lhs, const auto& rhs) {
      return lhs.FileGuestEnd <= rhs.FileGuestBegin;
    });
