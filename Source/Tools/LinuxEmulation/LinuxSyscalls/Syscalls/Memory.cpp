...
void RegisterMemory(FEX::HLE::SyscallHandler* Handler) {
  ...
  REGISTER_SYSCALL_IMPL_PASS_FLAGS(mbind, SyscallFlags::OPTIMIZETHROUGH | SyscallFlags::NOSYNCSTATEONENTRY,
                                   [](FEXCore::Core::CpuStateFrame* Frame, void* addr, unsigned long len, int mode,
                                      const unsigned long* nodemask, unsigned long maxnode, unsigned flags) -> uint64_t {
  ...
  REGISTER_SYSCALL_IMPL_PASS_FLAGS(get_mempolicy, SyscallFlags::OPTIMIZETHROUGH | SyscallFlags::NOSYNCSTATEONENTRY,
                                   [](FEXCore::Core::CpuStateFrame* Frame, int* mode, unsigned long* nodemask, unsigned long maxnode,
                                      void* addr, unsigned long flags) -> uint64_t {
  ...
  REGISTER_SYSCALL_IMPL_PASS_FLAGS(migrate_pages, SyscallFlags::OPTIMIZETHROUGH | SyscallFlags::NOSYNCSTATEONENTRY,
                                   [](FEXCore::Core::CpuStateFrame* Frame, int pid, unsigned long maxnode, const unsigned long* old_nodes,
                                      const unsigned long* new_nodes) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(migrate_pages), pid, maxnode, old_nodes, new_nodes);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_PASS_FLAGS(
    move_pages, SyscallFlags::OPTIMIZETHROUGH | SyscallFlags::NOSYNCSTATEONENTRY,
  ...
  if (Handler->IsHostKernelVersionAtLeast(5, 17, 0)) {
    REGISTER_SYSCALL_IMPL_PASS_FLAGS(
      set_mempolicy_home_node, SyscallFlags::OPTIMIZETHROUGH | SyscallFlags::NOSYNCSTATEONENTRY,
