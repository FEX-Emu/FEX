...
void RegisterSocket(FEX::HLE::SyscallHandler* Handler) {
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(recvmmsg,
                                 [](FEXCore::Core::CpuStateFrame* Frame, int sockfd, struct mmsghdr* msgvec, unsigned int vlen, int flags,
                                    struct timespec* timeout) -> uint64_t {
    uint64_t Result = ::recvmmsg(sockfd, msgvec, vlen, flags, timeout);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(
    sendmmsg, [](FEXCore::Core::CpuStateFrame* Frame, int sockfd, struct mmsghdr* msgvec, uint32_t vlen, int flags) -> uint64_t {
