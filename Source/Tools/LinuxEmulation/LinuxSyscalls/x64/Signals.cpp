...
auto rt_sigaction(FEXCore::Core::CpuStateFrame* Frame, int signum, const GuestSigAction* act, GuestSigAction* oldact, size_t sigsetsize) -> uint64_t {
  if (sigsetsize != 8) {
    return -EINVAL;
  }
...
auto rt_sigtimedwait(FEXCore::Core::CpuStateFrame* Frame, uint64_t* set, siginfo_t* info, const struct timespec* timeout, size_t sigsetsize)
  -> uint64_t {
...
void RegisterSignals(FEX::HLE::SyscallHandler* Handler) {
  REGISTER_SYSCALL_IMPL_X64(rt_sigaction, rt_sigaction);
  REGISTER_SYSCALL_IMPL_X64(rt_sigtimedwait, rt_sigtimedwait);
}
} // namespace FEX::HLE::x64
