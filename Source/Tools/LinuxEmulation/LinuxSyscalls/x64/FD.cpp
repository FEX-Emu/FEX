...
void RegisterFD(FEX::HLE::SyscallHandler* Handler) {
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(preadv, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l, uint64_t pos_h) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(preadv), fd, iov, vlen, pos_l, pos_h);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(pwritev, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l, uint64_t pos_h) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(pwritev), fd, iov, vlen, pos_l, pos_h);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(preadv2, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l, uint64_t pos_h, int flags) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(preadv2), fd, iov, vlen, pos_l, pos_h, flags);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(pwritev2, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l, uint64_t pos_h, int flags) -> uint64_t {
