...
void RegisterFD(FEX::HLE::SyscallHandler* Handler) {
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(
    utimensat, [](FEXCore::Core::CpuStateFrame* Frame, int dirfd, const char* pathname, const struct timespec times[2], int flags) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(utimensat), dirfd, pathname, times, flags);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(pselect6,
                                 [](FEXCore::Core::CpuStateFrame* Frame, int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds,
                                    const struct timespec* timeout, const void* sigmaskpack) -> uint64_t {
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(
    vmsplice, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, unsigned long nr_segs, unsigned int flags) -> uint64_t {
    uint64_t Result = ::vmsplice(fd, iov, nr_segs, flags);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(
    preadv, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l, uint64_t pos_h) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(preadv), fd, iov, vlen, pos_l, pos_h);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(
    pwritev, [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l, uint64_t pos_h) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(pwritev), fd, iov, vlen, pos_l, pos_h);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(preadv2,
                                 [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l,
                                    uint64_t pos_h, int flags) -> uint64_t {
    uint64_t Result = ::syscall(SYSCALL_DEF(preadv2), fd, iov, vlen, pos_l, pos_h, flags);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(pwritev2,
                                 [](FEXCore::Core::CpuStateFrame* Frame, int fd, const struct iovec* iov, uint64_t vlen, uint64_t pos_l,
                                    uint64_t pos_h, int flags) -> uint64_t {
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(process_vm_readv,
                                 [](FEXCore::Core::CpuStateFrame* Frame, pid_t pid, const struct iovec* local_iov, unsigned long liovcnt,
                                    const struct iovec* remote_iov, unsigned long riovcnt, unsigned long flags) -> uint64_t {
    uint64_t Result = ::process_vm_readv(pid, local_iov, liovcnt, remote_iov, riovcnt, flags);
    SYSCALL_ERRNO();
  });

  REGISTER_SYSCALL_IMPL_X64_PASS(process_vm_writev,
                                 [](FEXCore::Core::CpuStateFrame* Frame, pid_t pid, const struct iovec* local_iov, unsigned long liovcnt,
                                    const struct iovec* remote_iov, unsigned long riovcnt, unsigned long flags) -> uint64_t {
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(ppoll,
                                 [](FEXCore::Core::CpuStateFrame* Frame, struct pollfd* fds, nfds_t nfds, struct timespec* timeout_ts,
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(getdents64, [](FEXCore::Core::CpuStateFrame* Frame, int fd, void* dirp, uint32_t count) -> uint64_t {
    uint64_t Result = syscall(SYSCALL_DEF(getdents64), static_cast<uint64_t>(fd), reinterpret_cast<uint64_t>(dirp), static_cast<uint64_t>(count));
    SYSCALL_ERRNO();
  });
  ...
  REGISTER_SYSCALL_IMPL_X64_PASS(
    sync_file_range, [](FEXCore::Core::CpuStateFrame* Frame, int fd, off64_t offset, off64_t nbytes, unsigned int flags) -> uint64_t {
