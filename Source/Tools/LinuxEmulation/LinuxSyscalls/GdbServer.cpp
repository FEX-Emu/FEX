...
constexpr std::array< const std::string_view, 22> FlagNames = {
  "CF", "", "PF", "", "AF", "", "ZF", "SF", "TF", "IF", "DF", "OF", "IOPL", "", "NT", "", "RF", "VM", "AC", "VIF", "VIP", "ID",
};

static const std::string_view& GetFlagName(unsigned Flag) { return FlagNames[Flag]; }

static const std::string_view GetGRegName(unsigned Reg) {
  switch (Reg) {
...
static fextl::string encodeHex(std::string_view str) { return encodeHex(reinterpret_cast<const unsigned char*>(str.data()), str.size()); }
...
void GdbServer::SendPacketPair(const HandledPacketType& response) {
  ...
  if (response.TypeResponse == HandledPacketType::TYPE_UNKNOWN) {
    ...
    response.TypeResponse != HandledPacketType::TYPE_ONLYNACK && response.TypeResponse != HandledPacketType::TYPE_ONLYACK && response.TypeResponse != HandledPacketType::TYPE_NONE) {
