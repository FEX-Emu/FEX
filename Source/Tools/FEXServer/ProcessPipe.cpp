 ...
 */
static void AggregateCodeMaps(const FEXCore::ExecutableFileInfo& MainFileId, bool HasMultiblock) {
  std::set<FEXCore::ExecutableFileInfo> Result;
  ...
  // Read all dependencies discovered in previous runs
  {
    ...
    for (auto& [FileId, Contents] : FEXCore::CodeMap::ParseCodeMap(MainFileCodeMap)) {
      Result.emplace(nullptr, FileId, Contents.Filename);
    }
  }
  ...
  for (auto& [File, _] : IncomingCodeMap) {
    Result.emplace(nullptr, File.FileId, File.Filename);
  }
  ...
  for (auto& [File, Blocks] : IncomingCodeMap) {
    ...
    // Update code map
    std::set<FEXCore::ExecutableFileInfo> Empty;
...
void HandleSocketData(fasio::tcp_socket& Socket) {
  ...
  while (buffer.size() > 0) {
    ...
    case FEXServerClient::PacketType::TYPE_POPULATE_CODE_CACHE_NO_MULTIBLOCK: {
      ...
      // Update code maps
      AggregateCodeMaps(MainFileId, HasMultiblock);
