...
private:
  ...
  std::unordered_map<IR::NodeWrapper::NodeOffsetType, Label> JumpTargets;
   ...
   * @{ */
  constexpr static uint32_t NumGPRs = 11;
  constexpr static uint32_t NumXMMs = 11;
  ...
  constexpr static uint32_t XMMClass = 1;
...
JITCore::JITCore(FEXCore::Context::Context *ctx)
  ...
  RA::AddRegisters(RASet, XMMClass, XMMBase, NumXMMs);
  ...
  while (Begin != End) {
    ...
    if (IROp->HasDest) {
      ...
      case OP_LOADCONTEXT: {
        ...
        if (Op->Size == 16)
          RA::SetNodeClass(Graph, WrapperOp->ID(), XMMClass);
        else
      ...
      case OP_CPUID: RA::SetNodeClass(Graph, WrapperOp->ID(), XMMClass); break;
      default:
  ...
  while (Begin != End) {
   ...
   break;
    case IR::OP_JUMP: {
      ...
      if (IsTarget == JumpTargets.end()) {
        TargetLabel = &JumpTargets.try_emplace(Op->Header.Args[0].ID(), Label{}).first->second;
      }
      else {
    ...
    if (HasRA) {
      ...
      case IR::OP_CONDJUMP: {
        ...
        if (IsTarget == JumpTargets.end()) {
          TargetLabel = &JumpTargets.try_emplace(Op->Header.Args[1].ID(), Label{}).first->second;
        }
        else {
          TargetLabel = &IsTarget->second;
        }
...
#undef LOAD_CTX
  ...
  ready();
//  LogMan::Msg::D("Ptr: %p,+%ld", Entry, getCurr<uintptr_t>() - (uintptr_t)Entry);
//	static int a = 0;
//	if (a++ > 100)
//		__builtin_trap();
  return Entry;
