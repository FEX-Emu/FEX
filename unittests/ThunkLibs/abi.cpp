...
TEST_CASE_METHOD(Fixture, "DataLayout") {
    ...
    SECTION("Type with platform-dependent size (size_t)") {
        ...
        REQUIRE(action->guest_layout->contains("A"));
        CHECK(action->GetTypeCompatibility("struct A") == (guest_abi == GuestABI::X86_32 ? TypeCompatibility::Repackable : TypeCompatibility::Full));
    }
...
// TODO: Currently unsupported by the generator
...
if (guest_abi == GuestABI::X86_32) // TODO
    SECTION("TODO") {
        // Without annotation
        auto action = compute_data_layout(
            "#include <thunks_common.h>\n"
