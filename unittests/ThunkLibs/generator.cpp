...
struct Fixture {
  ...
  ~Fixture() { std::filesystem::remove_all(tmpdir); }
...
public:
  ...
  bool matched() const noexcept { return success; }
};
...
HasASTMatching<DeclarationMatcher> matches(const DeclarationMatcher& matcher_) { return HasASTMatching<DeclarationMatcher>(matcher_); }

HasASTMatching<StatementMatcher> matches(const StatementMatcher& matcher_) { return HasASTMatching<StatementMatcher>(matcher_); }
...
SourceWithAST::SourceWithAST(std::string_view input, bool silent_compile)
  ...
  struct ToolAction : clang::tooling::ToolAction {
    ...
    bool
    runInvocation(std::shared_ptr<clang::CompilerInvocation> invocation, clang::FileManager* files, std::shared_ptr<clang::PCHContainerOperations> pch, clang::DiagnosticConsumer* diag_consumer) override {
...
TEST_CASE_METHOD(Fixture, "Trivial") {
  ...
  CHECK_THAT(
    output.host,
    matches(varDecl(
      hasName("exports"), hasType(constantArrayType(hasElementType(asStructString("ExportEntry")), hasSize(2))),
      hasInitializer(initListExpr(hasInit(0, expr()), hasInit(1, initListExpr(hasInit(0, implicitCastExpr()), hasInit(1, implicitCastExpr())))))
...
TEST_CASE_METHOD(Fixture, "VersionedLibrary") {
  ...
  CHECK_THAT(
    output,
    matches(callExpr(callee(functionDecl(hasName("dlopen"))), hasArgument(0, stringLiteral().bind("libname"))))
      .check_binding(
...
TEST_CASE_METHOD(Fixture, "FunctionPointerParameter") {
  ...
  CHECK_THAT(
    ...
    matches(functionDecl(
      // Should have signature matching input function
      hasName("fexfn_pack_func"), returns(asString("void")), parameterCountIs(1), hasParameter(0, hasType(asString("int (*)(char, char)"))))));
  ...
  CHECK_THAT(
    ...
    matches(
      functionDecl(
        ...
        hasDescendant(callExpr(callee(functionDecl(hasName("FinalizeHostTrampolineForGuestFunction"))), hasArgument(0, expr().bind("funcpt"
                                                                                                                                   "r"))))))
      .check_binding(
  ...
  CHECK_THAT(output.host, matches(varDecl(hasName("exports"), hasType(constantArrayType(hasElementType(asStructString("ExportEntry")), hasSize(3))), hasInitializer(hasDescendant(declRefExpr(to(cxxMethodDecl(hasName("Call"), ofClass(hasName("GuestWrapperForHostFunction"))))))))));
}
...
TEST_CASE_METHOD(Fixture, "MultipleParameters") {
  ...
  CHECK_THAT(output.guest, matches(functionDecl(hasName("fexfn_pack_func"), returns(asString("void")), parameterCountIs(4), hasParameter(0, hasType(asString("int"))), hasParameter(1, hasType(asString("char"))), hasParameter(2, hasType(asString("unsigned long"))), hasParameter(3, hasType(asStructString("TestStruct"))))));
  ...
  CHECK_THAT(
    ...
    matches(functionDecl(
      ...
      parameterCountIs(1), hasParameter(0, hasType(pointerType(pointee(hasUnqualifiedDesugaredType(recordType(hasDeclaration(decl(has(fieldDecl(hasType(asString("guest_layout<int32_t>")))), has(fieldDecl(hasType(asString("guest_layout<uint8_t>")))), has(fieldDecl(hasType(asString("guest_layout<uint64_t>")))), has(fieldDecl(hasType(asString("guest_layout<" CLANG_STRUCT_PREFIX "TestStruct>")))))))))))))));
}
...
TEST_CASE_METHOD(Fixture, "VariadicFunction") {
  ...
  CHECK_THAT(output, matches(functionDecl(hasName("fexfn_pack_func_internal"), returns(asString("void")), parameterCountIs(3), hasParameter(0, hasType(asString("int"))), hasParameter(1, hasType(asString("unsigned long"))), hasParameter(2, hasType(pointerType(pointee(asString("char"))))))));
}
...
TEST_CASE_METHOD(Fixture, "LayoutWrappers") {
  ...
  const auto host_layout_is_trivial = matches(classTemplateSpecializationDecl(
    hasName("host_layout"), hasAnyTemplateArgument(refersToType(asString("struct A"))), has(fieldDecl(hasName("data"), hasType(hasCanonicalType(asString("struct A")))))));
  ...
  const auto guest_converter_undefined = matches(functionDecl(
    hasName("to_guest"),
    // Parameter is a host_layout<A> (ignoring qualifiers and references)
    hasParameter(0, hasType(references(classTemplateSpecializationDecl(hasName("host_layout"), hasAnyTemplateArgument(refersToType(asString("struct A"))))))), isDeleted()));
  ...
  SECTION("Fully compatible type") {
    ...
    CHECK_THAT(output, matches(classTemplateSpecializationDecl(hasName("guest_layout"), hasAnyTemplateArgument(refersToType(asString("struct A"))), has(fieldDecl(hasName("data"), hasType(hasCanonicalType(asString("struct A"))))))));
      ...
      matches(classTemplateSpecializationDecl(
        ...
        has(fieldDecl(
          ...
          hasType(hasCanonicalType(hasDeclaration(decl(
            ...
            has(fieldDecl(hasName("a"), hasType(asString("guest_layout<" CLANG_STRUCT_PREFIX "B *>")))), has(fieldDecl(hasName("b"), hasType(asString("guest_layout<int32_t>")))))))))))));
    CHECK_THAT(output, guest_converter_defined);
...
TEST_CASE_METHOD(Fixture, "Mapping guest integers to fixed-size") {
    ...
    if (!ptr.empty() && guest_abi == GuestABI::X86_32 && !passthrough_guest_type) {
      ...
      CHECK_THAT(
        ...
        matches(functionDecl(
          hasName("fexfn_unpack_libtest_func"),
          // Packed argument struct should contain all parameters
          parameterCountIs(1), hasParameter(0, hasType(pointerType(pointee(hasUnqualifiedDesugaredType(recordType(hasDeclaration(decl(has(fieldDecl(hasType(asString(expected_type)))))))))))))));

      // For passthrough parameters, the target function signature should
      // match the guest_layout type
      if (passthrough_guest_type) {
        CHECK_THAT(output, matches(functionDecl(hasName("fexfn_impl_libtest_func"), parameterCountIs(1), hasParameter(0, hasType(asString(expected_type))))));
      }
    }
  }
  ...
  SECTION("Special integer types") {
    ...
    // match the guest_layout type
    if (passthrough_guest_type) {
      CHECK_THAT(output, matches(functionDecl(hasName("fexfn_impl_libtest_func"), parameterCountIs(1), hasParameter(0, hasType(asString("guest_layout<" + fixed_size_type + ptr + ">"))))));
    }
  }
}
...
TEST_CASE_METHOD(Fixture, "StructRepacking") {
  ...
  SECTION("Pointer to struct with unannotated pointer member with inconsistent data layout") {
    ...
    SECTION("Parameter unannotated") { CHECK_THROWS(run_thunkgen_host(prelude, code, guest_abi, true)); }
