...
def HandleFunctionDeclCursor(Arch, Cursor):
    ...
    for Child in Cursor.get_children():
        if Child.kind == CursorKind.TYPE_REF:
            ...
            Child.kind == CursorKind.VISIBILITY_ATTR
            or Child.kind == CursorKind.UNEXPOSED_ATTR
            or Child.kind == CursorKind.CONST_ATTR
            or Child.kind == CursorKind.PURE_ATTR
        ):
            pass
        else:
            logging.critical(
                "\tUnhandled FunctionDeclCursor {0}-{1}-{2}".format(
...
def main():
    ...
    args_x86_64 = [
        ...
        "-isystem",
        "/usr/x86_64-linux-gnu/include/c++/10/x86_64-linux-gnu/",
        "-isystem",
