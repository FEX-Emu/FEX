...
def FindClangArguments(OriginalArguments):
    ...
    Proc = subprocess.Popen(
        AddedArguments, stderr=subprocess.PIPE, stdin=subprocess.DEVNULL
    )
    NewIncludes = []
    BeginSearch = False
    while True:
...
def HandleStructDeclCursor(Arch, Cursor, NameOverride=""):
    ...
    Struct = StructDefinition(Name=CursorName, Size=StructType.get_size())
...
def HandleUnionDeclCursor(Arch, Cursor, NameOverride=""):
    ...
    Union = UnionDefinition(Name=CursorName, Size=UnionType.get_size())
...
def HandleVarDeclCursor(Arch, Cursor):
    ...
    VarDecl = VarDeclDefinition(Name=CursorName, Size=DeclType.get_size())
        ...
        if Child.kind == CursorKind.ANNOTATE_ATTR:
            if Child.spelling.startswith("ioctl-alias-"):
                ...
                if Sections[2] == "x86_32":
                    VarDecl.Aliases.append(
                        AliasType(Sections[3], AliasType.ALIAS_X86_32)
                    )
                elif Sections[2] == "x86_64":
                    VarDecl.Aliases.append(
                        AliasType(Sections[3], AliasType.ALIAS_X86_64)
                    )
                elif Sections[2] == "aarch64":
                    VarDecl.Aliases.append(
                        AliasType(Sections[3], AliasType.ALIAS_AARCH64)
                    )
                elif Sections[2] == "win32":
                    VarDecl.Aliases.append(
                        AliasType(Sections[3], AliasType.ALIAS_WIN32)
                    )
                elif Sections[2] == "win64":
                    VarDecl.Aliases.append(
                        AliasType(Sections[3], AliasType.ALIAS_WIN64)
                    )
                else:
                    logging.critical(
                        "Can't handle alias type '{0}'".format(Child.spelling)
                    )
                    Arch.Parsed = False
            ...
            Child.kind == CursorKind.TYPE_REF
            or Child.kind == CursorKind.UNEXPOSED_EXPR
            or Child.kind == CursorKind.PAREN_EXPR
            or Child.kind == CursorKind.BINARY_OPERATOR
        ):
            pass
...
def HandleTypeDefDeclCursor(Arch, Cursor):
    ...
    if (
        TypeDefType.kind == TypeKind.ELABORATED
        and CanonicalType.kind == TypeKind.RECORD
    ):
        ...
        if len(TypeDefName) != 0:
            ...
            VarDecl = VarDeclDefinition(Name=TypeDefName, Size=CanonicalType.get_size())
            Arch.VarDecls[VarDecl.Name] = HandleVarDeclElements(Arch, VarDecl, Cursor)
        ...
        if Child.kind == CursorKind.ANNOTATE_ATTR:
            if Child.spelling.startswith("alias-"):
                ...
                if Sections[1] == "x86_32":
                    Struct.Aliases.append(
                        AliasType(Sections[2], AliasType.ALIAS_X86_32)
                    )
                elif Sections[1] == "x86_64":
                    Struct.Aliases.append(
                        AliasType(Sections[2], AliasType.ALIAS_X86_64)
                    )
                elif Sections[1] == "aarch64":
                    Struct.Aliases.append(
                        AliasType(Sections[2], AliasType.ALIAS_AARCH64)
                    )
...
def HandleTypeDefDecl(Arch, Cursor, Name):
    for Child in Cursor.get_children():
        if Child.kind == CursorKind.UNION_DECL:
            ...
            Child.kind == CursorKind.TYPE_REF
            or Child.kind == CursorKind.NAMESPACE_REF
            or Child.kind == CursorKind.TEMPLATE_REF
            or Child.kind == CursorKind.ALIGNED_ATTR
        ):
            # Safe to pass on
            pass
        else:
            logging.critical(
                "Unhandled TypedefDecl {0}-{1}-{2}".format(
                    Child.kind, Child.type.spelling, Child.spelling
                )
            )
...
def PrintMissingMembers(Struct1, Struct2):
    for Member1 in Struct1.Members:
        ...
        if WasMissing:
            logging.error(
                "\t'{0}' member '{1}' Doesn't exist in '{2}'".format(
                    Struct1.Name, Member1.Name, Struct2.Name
                )
            )
...
def CompareStructs(Struct1, Struct2):
    ...
    if Struct1.Size != Struct2.Size:
        logging.warning(
            "\t#### Warning: Struct size mismatch. {0} != {1}".format(
                Struct1.Size, Struct2.Size
            )
        )
        logging.warning(
            "\t\tMight not be a problem if struct isn't in used inside another struct, or end of object"
        )
        HadWarning = True
    ...
    if len(Struct1.Members) != len(Struct2.Members):
        logging.error(
            "@@@@ ERROR: Struct fields mismatch! Number of fields don't match! {0} != {1}".format(
                len(Struct1.Members), len(Struct2.Members)
            )
        )
        ...
        for StructMemberIndex in range(0, len(Struct1.Members)):
            ...
            if Member1.Type == TypeDefinition.TYPE_FIELD:
                if Member1.Size != Member2.Size:
                    logging.error(
                        "\t@@@@ ERROR: Member '{0}' mismatch Size! {1} != {2}".format(
                            Member1.Name, Member1.Size, Member2.Size
                        )
                    )
                    HadError = True
                if Member1.OffsetOf != Member2.OffsetOf:
                    logging.error(
                        "\t@@@@ ERROR: Member '{0}' mismatch OffsetOf! {1} != {2}".format(
                            Member1.Name, Member1.OffsetOf, Member2.OffsetOf
                        )
                    )
                    HadError = True
                if Member1.Alignment != Member2.Alignment:
                    logging.error(
                        "\t@@@@ ERROR: Member '{0}' mismatch Alignment! {1} != {2}".format(
                            Member1.Name, Member1.Alignment, Member2.Alignment
                        )
                    )
                    logging.error(
                        "\t\tProbably not a problem if offset and size matches"
                    )
                    HadWarning = True
            else:
                logging.critical(
                    "Oops, didn't handle member type {0}".format(Member1.Type)
                )
        pass
...
def CompareAliases(DB, DBs):
    ...
    for StructKey, StructDef in DB.Structs.items():
        ...
        for Alias in StructDef.Aliases:
            ...
            if OtherStruct == None:
                logging.critical(
                    "Couldn't find alias {0} in {1} DB".format(
                        Alias.Name, OtherDB.ArchName
                    )
                )
                Passed = False
                continue
            ...
            if not (ThisAlias):
                logging.error(
                    "Couldn't Alias to Arch {0} successfully".format(OtherDB.ArchName)
                )
            Passed &= ThisAlias
    ...
    for VarDeclKey, VarDecl in DB.VarDecls.items():
        ...
        for Alias in VarDecl.Aliases:
            ...
            if VarDecl.Size != OtherAlias.Size:
                logging.critical(
                    "VarDecl: {0}/{1} didn't match {2}/{3}: {4:08X} != {5:08X}".format(
                        VarDeclKey,
                        DB.ArchName,
                        Alias.Name,
                        OtherDB.ArchName,
...
def CompareCrossArch(DB1, DB2):
    ...
    for StructKey, StructDef in DB1.Structs.items():
        ...
        if StructDef.ExpectFEXMatch:
            ...
            if Struct2 == None:
                logging.critical(
                    "Couldn't find Struct {0} in {1} DB".format(
                        StructDef.Name, DB2.ArchName
                    )
                )
                Passed = False
                continue
...
def main():
    ...
    DBs = DBList(Arch_x86_32, Arch_x86_64, Arch_aarch64, Arch_x86_win32, Arch_x86_win64)
