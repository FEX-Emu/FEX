...
def update_performance_numbers(
    ...
    for key, items in new_json_numbers.items():
        ...
        if "ExpectedArm64ASM" in items:
            performance_json["Instructions"][key]["ExpectedArm64ASM"] = items[
                "ExpectedArm64ASM"
            ]
...
def main():
    ...
    if len(sys.argv) < 3:
        logging.critical(
            "usage: %s <PerformanceTests.json> <NewNumbers.json>" % (sys.argv[0])
        )
    ...
    try:
        ...
        return update_performance_numbers(
            performance_json_path, performance_json_data, new_json_numbers_data
        )
