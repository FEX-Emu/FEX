...
def print_options(options):
    for op_group, group_vals in options.items():
        for op_key, op_vals in group_vals.items():
            ...
            print_config(op_vals["Type"], op_group, op_key, default)
...
def print_man_environment(options):
    ...
    for op_group, group_vals in options.items():
        for op_key, op_vals in group_vals.items():
            ...
            if (
                value_type == "str"
                or value_type == "strarray"
                or value_type == "strenum"
            ):
                # Wrap the string argument in quotes
                default = "'" + default + "'"
            print_man_env_option(op_key, op_vals["Desc"], default, False)

            if value_type == "strenum":
                Enums = op_vals["Enums"]
                output_man.write("\\fBAvailable Options:\\fR\n")
                output_man.write(
                    ", ".join(f"{enum_op_val}" for [_, enum_op_val] in Enums.items())
                )
                output_man.write("\n.sp\n")
    ...
    for op_group, group_vals in options.items():
        for op_key, op_vals in group_vals.items():
            ...
            if value_type == "strenum":
                ...
                output_argloader.write(
                    "\tValue = FEXCore::Config::EnumParser<FEXCore::Config::{}ConfigPair>(FEXCore::Config::{}_EnumPairs, Value_View);\n".format(
                        op_key, op_key
                    )
                )
                output_argloader.write("}\n")
            ...
            if "ArgumentHandler" in op_vals:
                conversion_func = "FEXCore::Config::Handler::{0}".format(
                    op_vals["ArgumentHandler"]
                )
                output_argloader.write(
                    'else if (Key == "FEX_{0}") {{\n'.format(op_key.upper())
                )
    ...
    for op_group, group_vals in options.items():
        for op_key, op_vals in group_vals.items():
            ...
            if value_type == "strenum":
                ...
                output_argloader.write(
                    "\tSet(KeyOption, FEXCore::Config::EnumParser<FEXCore::Config::{}ConfigPair>(FEXCore::Config::{}_EnumPairs, Value_View));\n".format(
                        op_key, op_key
                    )
                )
                output_argloader.write("}\n")
    ...
    for op_group, group_vals in options.items():
        for op_key, op_vals in group_vals.items():
            if op_vals["Type"] == "strenum":
                ...
                output_argloader.write(
                    "using {}ConfigPair = std::pair<std::string_view, FEXCore::Config::{}>;\n".format(
                        op_key, op_key
                    )
                )
                output_argloader.write(
                    "constexpr static std::array<{}ConfigPair, {}> {}_EnumPairs = {{{{\n".format(
                        op_key, len(Enums) + 1, op_key
                    )
                )
                i = 0
                # Always have an OFF.
                output_argloader.write(
                    '\t{{ "off", FEXCore::Config::{}::OFF }},\n'.format(op_key)
                )
                for enum_op_key, enum_op_vals in Enums.items():
                    output_argloader.write(
                        '\t{{ "{}", FEXCore::Config::{}::{} }},\n'.format(
                            enum_op_vals, op_key, enum_op_key.upper()
                        )
                    )
                    i += 1
