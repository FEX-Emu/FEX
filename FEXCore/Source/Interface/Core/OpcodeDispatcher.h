...
#undef OpcodeArgs
  ...
  struct AddressMode AddSegmentToAddress(struct AddressMode A, uint32_t Flags);
  OrderedNode* LoadEffectiveAddress(struct AddressMode A, bool AllowUpperGarbage = false);
  struct AddressMode SelectAddressMode(struct AddressMode A, bool AtomicTSO, bool Vector, unsigned AccessSize);
...
#define DeriveOp(Dest, NewOp, Expr)                \
  ...
  OrderedNode*
  _LoadMemAutoTSO(FEXCore::IR::RegisterClassType Class, uint8_t Size, struct AddressMode A, uint8_t Align = 1, bool ForceNonTSO = false) {
  ...
  OrderedNode* _StoreMemAutoTSO(FEXCore::IR::RegisterClassType Class, uint8_t Size, struct AddressMode A, OrderedNode* Value,
                                uint8_t Align = 1, bool ForceNonTSO = false) {
    bool AtomicTSO = CTX->IsAtomicTSOEnabled() && !ForceNonTSO;
    A = SelectAddressMode(A, AtomicTSO, Class != GPRClass, Size);
