...
Ref OpDispatchBuilder::AVX128_HSUBPImpl(size_t ElementSize, Ref Src1, Ref Src2) {
  auto Even = _VUnZip(OpSize::i128Bit, ElementSize, Src1, Src2);
  auto Odd = _VUnZip2(OpSize::i128Bit, ElementSize, Src1, Src2);
  return _VFSub(OpSize::i128Bit, ElementSize, Even, Odd);
}

template<size_t ElementSize>
void OpDispatchBuilder::AVX128_VHSUBP(OpcodeArgs) {
  AVX128_VectorBinaryImpl(Op, GetDstSize(Op), ElementSize,
                          [this](size_t, Ref Src1, Ref Src2) { return AVX128_HSUBPImpl(ElementSize, Src1, Src2); });
}
...
void OpDispatchBuilder::AVX128_VPALIGNR(OpcodeArgs) {
  ...
  const auto Index = Op->Src[2].Data.Literal.Value;
...
void OpDispatchBuilder::AVX128_VTESTP(OpcodeArgs) {
  ...
  const auto MaskConstant = 1ULL << (ElementSizeInBits - 1);
  ...
  auto GenerateResult128 = [this](Ref Src1, Ref Src2) {
    Ref Mask = _VDupFromGPR(OpSize::i128Bit, ElementSize, _Constant(MaskConstant));

    Ref AndTest = _VAnd(OpSize::i128Bit, 1, Src2, Src1);
    Ref AndNotTest = _VAndn(OpSize::i128Bit, 1, Src2, Src1);

    Ref MaskedAnd = _VAnd(OpSize::i128Bit, 1, AndTest, Mask);
    Ref MaskedAndNot = _VAnd(OpSize::i128Bit, 1, AndNotTest, Mask);

    Ref MaxAnd = _VUMaxV(OpSize::i128Bit, 2, MaskedAnd);
    Ref MaxAndNot = _VUMaxV(OpSize::i128Bit, 2, MaskedAndNot);

    Ref AndGPR = _VExtractToGPR(OpSize::i128Bit, 2, MaxAnd, 0);
    Ref AndNotGPR = _VExtractToGPR(OpSize::i128Bit, 2, MaxAndNot, 0);
  ...
  auto GenerateResult256 = [this](RefPair Src1, RefPair Src2) {
  ...
  FlagResults Result {};
  if (Is128Bit) {
    Result = GenerateResult128(Src1.Low, Src2.Low);
  } else {
    Result = GenerateResult256(Src1, Src2);
  }
  // As in PTest, this sets Z appropriately while zeroing the rest of NZCV.
  SetNZ_ZeroCV(32, Result.ZF);
...
void OpDispatchBuilder::AVX128_PTest(OpcodeArgs) {
  ...
  auto GenerateResult128 = [this](Ref Src1, Ref Src2) {
    Ref Test1 = _VAnd(OpSize::i128Bit, OpSize::i8Bit, Src1, Src2);
    Ref Test2 = _VAndn(OpSize::i128Bit, OpSize::i8Bit, Src2, Src1);

    // Element size must be less than 32-bit for the sign bit tricks.
    Test1 = _VUMaxV(OpSize::i128Bit, OpSize::i16Bit, Test1);
    Test2 = _VUMaxV(OpSize::i128Bit, OpSize::i16Bit, Test2);
