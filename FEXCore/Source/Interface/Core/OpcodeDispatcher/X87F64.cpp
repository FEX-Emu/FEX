...
void OpDispatchBuilder::X87FXTRACTF64(OpcodeArgs) {
  ...
  Ref MaskedGpr = _And(OpSize::i64Bit, Gpr, _Constant(0x7fffffffffffffffUL));

  // zero case
  Ref ExpZV = _VCastFromGPR(8, 8, _Constant(0xfff0000000000000UL));
  Ref SigZV = Node;
  ...
  SaveNZCV();
  _SubNZCV(OpSize::i64Bit, MaskedGpr, _Constant(0));

  Ref Sig = _NZCVSelectV(8, CondClassType {.Val = FEXCore::IR::COND_EQ}, SigZV, SigNZV);
  Ref Exp = _NZCVSelectV(8, CondClassType {.Val = FEXCore::IR::COND_EQ}, ExpZV, ExpNZV);
