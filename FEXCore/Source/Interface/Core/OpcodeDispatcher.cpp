...
void OpDispatchBuilder::SyscallOp(OpcodeArgs) {
  ...
  if (OSABI != FEXCore::HLE::SyscallOSABI::OS_HANGOVER && (DefaultSyscallFlags & FEXCore::IR::SyscallFlags::NORETURNEDRESULT) != FEXCore::IR::SyscallFlags::NORETURNEDRESULT) {
...
void OpDispatchBuilder::LEAOp(OpcodeArgs) {
  ...
  if (CTX->Config.Is64BitMode) {
    const uint32_t DstSize =
      X86Tables::DecodeFlags::GetOpAddr(Op->Flags, 0) == X86Tables::DecodeFlags::FLAG_OPERAND_SIZE_LAST ?
        2 :
      X86Tables::DecodeFlags::GetOpAddr(Op->Flags, 0) == X86Tables::DecodeFlags::FLAG_WIDENING_SIZE_LAST ?
        8 :
        4;
...
#define OPD(group, prefix, Reg) (((group - FEXCore::X86Tables::TYPE_GROUP_1) << 6) | (prefix) << 3 | (Reg))
  ...
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x83), 0): IROp = FEXCore::IR::IROps::OP_ADD; break;
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x80), 1):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x81), 1):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x83), 1): IROp = FEXCore::IR::IROps::OP_OR; break;
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x80), 4):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x81), 4):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x83), 4): IROp = FEXCore::IR::IROps::OP_ANDWITHFLAGS; break;
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x80), 5):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x81), 5):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x83), 5): IROp = FEXCore::IR::IROps::OP_SUB; break;
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x80), 6):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x81), 6):
  case OPD(FEXCore::X86Tables::TYPE_GROUP_1, OpToIndex(0x83), 6): IROp = FEXCore::IR::IROps::OP_XOR; break;
  default:
...
#undef OPD
  ...
  if (DestIsLockedMem(Op)) {
    ...
    default: LOGMAN_MSG_A_FMT("Unknown Atomic IR Op: {}", ToUnderlying(IROp)); break;
    }
  } else {
  ...
  {
    switch (IROp) {
    case FEXCore::IR::IROps::OP_ADD: GenerateFlags_ADD(Op, Result, Dest, Src); break;
    case FEXCore::IR::IROps::OP_SUB: GenerateFlags_SUB(Op, Result, Dest, Src); break;
    case FEXCore::IR::IROps::OP_XOR:
    case FEXCore::IR::IROps::OP_OR: {
...
OrderedNode*
OpDispatchBuilder::SelectBit(OrderedNode* Cmp, bool TrueIsNonzero, IR::OpSize ResultSize, OrderedNode* TrueValue, OrderedNode* FalseValue) {
  ...
  return _NZCVSelect(ResultSize, TrueIsNonzero ? CondClassType {COND_NEQ} : CondClassType {COND_EQ}, TrueValue, FalseValue);
}
...
void OpDispatchBuilder::ROLOp(OpcodeArgs) {
  ...
  auto ALUOp = _Ror(Size == 64 ? OpSize::i64Bit : OpSize::i32Bit, Dest, _Sub(Size == 64 ? OpSize::i64Bit : OpSize::i32Bit, _Constant(Size, std::max(32U, Size)), Src));
...
void OpDispatchBuilder::MOVSOp(OpcodeArgs) {
  ...
  if (Op->Flags & (FEXCore::X86Tables::DecodeFlags::FLAG_REP_PREFIX | FEXCore::X86Tables::DecodeFlags::FLAG_REPNE_PREFIX)) {
    ...
    auto Result = _MemCpy(CTX->IsAtomicTSOEnabled(), Size, DstSegment ?: InvalidNode, SrcSegment ?: InvalidNode, DstAddr, SrcAddr, Counter, DF);
...
void OpDispatchBuilder::CMPXCHGPairOp(OpcodeArgs) {
  ...
  OrderedNode* ZFResult = _Select(FEXCore::IR::COND_EQ, CASResult, Expected, OneConst, ZeroConst);
...
uint32_t OpDispatchBuilder::GetSrcBitSize(X86Tables::DecodedOp Op) const { return GetSrcSize(Op) * 8; }

uint32_t OpDispatchBuilder::GetDstBitSize(X86Tables::DecodedOp Op) const { return GetDstSize(Op) * 8; }
...
OrderedNode*
OpDispatchBuilder::LoadSource_WithOpSize(RegisterClassType Class, const X86Tables::DecodedOp& Op, const X86Tables::DecodedOperand& Operand, uint8_t OpSize, uint32_t Flags, const LoadSourceOptions& Options) {
  LOGMAN_THROW_A_FMT(
    Operand.IsGPR() || Operand.IsLiteral() || Operand.IsGPRDirect() || Operand.IsGPRIndirect() || Operand.IsRIPRelative() || Operand.IsSIB(), "Unsupported Src type");
...
OrderedNode*
OpDispatchBuilder::LoadSource(RegisterClassType Class, const X86Tables::DecodedOp& Op, const X86Tables::DecodedOperand& Operand, uint32_t Flags, const LoadSourceOptions& Options) {
  const uint8_t OpSize = GetSrcSize(Op);
  return LoadSource_WithOpSize(Class, Op, Operand, OpSize, Flags, Options);
}

void OpDispatchBuilder::StoreResult_WithOpSize(FEXCore::IR::RegisterClassType Class, FEXCore::X86Tables::DecodedOp Op, const FEXCore::X86Tables::DecodedOperand& Operand, OrderedNode* const Src, uint8_t OpSize, int8_t Align, MemoryAccessType AccessType) {
  LOGMAN_THROW_A_FMT(
    Operand.IsGPR() || Operand.IsLiteral() || Operand.IsGPRDirect() || Operand.IsGPRIndirect() || Operand.IsRIPRelative() || Operand.IsSIB(), "Unsupported Dest type");
...
void OpDispatchBuilder::StoreResult(FEXCore::IR::RegisterClassType Class, FEXCore::X86Tables::DecodedOp Op, const FEXCore::X86Tables::DecodedOperand& Operand, OrderedNode* const Src, int8_t Align, MemoryAccessType AccessType) {
...
void OpDispatchBuilder::UnhandledOp(OpcodeArgs) { DecodeFailure = true; }
...
void OpDispatchBuilder::ALUOpImpl(OpcodeArgs, FEXCore::IR::IROps ALUIROp, FEXCore::IR::IROps AtomicFetchOp) {
  ...
  if (DestIsLockedMem(Op)) {
     ...
     */
    if (ALUIROp == FEXCore::IR::IROps::OP_XOR && Op->Dest.IsGPR() && Op->Src[0].IsGPR() && Op->Dest.Data.GPR == Op->Src[0].Data.GPR) {
