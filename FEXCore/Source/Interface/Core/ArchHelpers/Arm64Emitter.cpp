...
namespace x32 {
  ...
  // Dynamic GPRs
  constexpr std::array<FEXCore::ARMEmitter::Register, 3> PreserveAll_Dynamic = {FEXCore::ARMEmitter::Reg::r16, FEXCore::ARMEmitter::Reg::r17, FEXCore::ARMEmitter::Reg::r30};
...
void Arm64Emitter::PushCalleeSavedRegisters() {
  ...
  for (auto& RegQuad : FPRs) {
    st4(ARMEmitter::SubRegSize::i64Bit, std::get<0>(RegQuad), std::get<1>(RegQuad), std::get<2>(RegQuad), std::get<3>(RegQuad), 0, ARMEmitter::Reg::r19, 32);
  }
}
...
void Arm64Emitter::PopCalleeSavedRegisters() {
  ...
  for (auto& RegQuad : FPRs) {
    ld4(ARMEmitter::SubRegSize::i64Bit, std::get<0>(RegQuad), std::get<1>(RegQuad), std::get<2>(RegQuad), std::get<3>(RegQuad), 0, ARMEmitter::Reg::rsp, 32);
  }
...
void Arm64Emitter::SpillStaticRegs(FEXCore::ARMEmitter::Register TmpReg, bool FPRs, uint32_t GPRSpillMask, uint32_t FPRSpillMask) {
  ...
  if (EmitterCTX->HostFeatures.SupportsAFP) {
    ...
    bic(
      ARMEmitter::Size::i64Bit, TmpReg, TmpReg,
      (1U << 2) |   // NEP
        (1U << 1)); // AH
