...
static uint64_t Arm64JITCore_ExitFunctionLink(FEXCore::Core::CpuStateFrame* Frame, FEXCore::Context::ExitFunctionLinkData* Record) {
  ...
  if (TFSet || !(HostCode = Thread->LookupCache->FindBlock(GuestRip))) {
...
void Arm64JITCore::EmitInterruptChecks(bool CheckTF) {
  if (CheckTF) {
    ...
    // Block TF for a single instruction when the frontend jumps to a new context.
    LoadConstant(ARMEmitter::Size::i32Bit, TMP1, 0b01);
