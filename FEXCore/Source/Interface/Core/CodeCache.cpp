...
bool CodeCache::LoadData(Core::InternalThreadState& Thread, std::byte* MappedCacheFile, const ExecutableFileSectionInfo& BinarySection) {
  ...
  fextl::set<uint64_t> EntrypointsSet;
  for (uint32_t i = 0; i < header.NumCodePages; ++i) {
    ...
    EntrypointsSet.clear();
    EntrypointsSet.insert(Entrypoints.begin(), Entrypoints.end());
    if (Thread.LookupCache->AddBlockExecutableRange(&Thread, EntrypointsSet, CodePage, FEXCore::Utils::FEX_PAGE_SIZE)) {
