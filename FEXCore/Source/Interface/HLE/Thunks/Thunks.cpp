...
struct TruncatingSHA256Hash {
  size_t operator()(const FEXCore::IR::SHA256Sum& SHA256Sum) const noexcept { return (const size_t&)SHA256Sum; }
};
...
struct ThunkHandler_impl final : public ThunkHandler {
  ...
  */
  ...
  static void LoadLib(void* ArgsV) {
    ...
    if (!Exports) {
      ERROR_AND_DIE_FMT(
        "LoadLib: Failed to initialize thunk library {}. "
  ...
  void RegisterTLSState(FEXCore::Core::InternalThreadState* _Thread) override { Thread = _Thread; }
...
fextl::unique_ptr<ThunkHandler> ThunkHandler::Create() { return fextl::make_unique<ThunkHandler_impl>(); }
...
HostToGuestTrampolinePtr* MakeHostTrampolineForGuestFunction(void* HostPacker, uintptr_t GuestTarget, uintptr_t GuestUnpacker) {
  ...
  if (ThunkHandler->HostTrampolineInstanceDataAvailable < HostToGuestTrampolineSize) {
    ...
    ThunkHandler->HostTrampolineInstanceDataPtr = (uint8_t*)mmap(0, ThunkHandler->HostTrampolineInstanceDataAvailable, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  ...
  GetInstanceInfo(HostTrampoline) = TrampolineInstanceInfo {.HostPacker = HostPacker, .CallCallback = (uintptr_t)&ThunkHandler_impl::CallCallback, .GuestUnpacker = GuestUnpacker, .GuestTarget = GuestTarget};
...
void FinalizeHostTrampolineForGuestFunction(HostToGuestTrampolinePtr* TrampolineAddress, void* HostPacker) {
  ...
  LOGMAN_THROW_A_FMT(Trampoline.CallCallback == (uintptr_t)&ThunkHandler_impl::CallCallback, "Invalid trampoline at {} passed to {}", fmt::ptr(TrampolineAddress), __FUNCTION__);
...
#else
fextl::unique_ptr<ThunkHandler> ThunkHandler::Create() { ERROR_AND_DIE_FMT("Unsupported"); }
#endif
