...
public:
  ...
  Ref InlineMemImmediate(OpSize Size, Ref Offset, MemOffsetType OffsetType, uint8_t& OffsetScale, bool TSO) {
    uint64_t Imm {};
...
#define DEF_INLINE(Type, Variable, Filter)                          \
  ...
  Ref InlineSubtractZero(OpSize Size, Ref Src1, Ref Src2) {
    ...
    // If we can inline the other source, prefer that. Otherwise inline a zero.
    return IsValueConstant(WrapNode(Src2), &_) ? Src1 : InlineZero(Size, Src1);
  }

  Ref InlineMem(OpSize Size, Ref Src, MemOffsetType OffsetType, uint8_t& OffsetScale) {
    return InlineMemImmediate(Size, Src, OffsetType, OffsetScale, false);
  }

  Ref InlineMemtso(OpSize Size, Ref Src, MemOffsetType OffsetType, uint8_t& OffsetScale) {
    return InlineMemImmediate(Size, Src, OffsetType, OffsetScale, true);
  }
#undef DEF_INLINE
