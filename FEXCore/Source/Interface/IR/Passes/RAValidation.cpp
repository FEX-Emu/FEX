...
struct RegState {
  ...
  void Spill(uint32_t SpillSlot, IR::NodeID ssa) { Spills[SpillSlot] = ssa; }
    ...
    for (auto [CodeNode, IROp] : CurrentIR.GetCode(BlockNode)) {
      ...
      const auto CheckArg = [&](uint32_t i, OrderedNodeWrapper Arg) {
        ...
        if (CurrentSSAAtReg == RegState::InvalidReg) {
          ...
          Errors << fextl::fmt::format(
            "%{}: Arg[{}] expects paired reg{} to contain %{}, but it actually contains {{%{}, %{}}}\n", ID, i, PhyReg.Reg, ArgID, Lower, Upper);
          ...
          Errors << fextl::fmt::format(
            "%{}: Arg[{}] expects reg{} to contain %{}, but contents vary depending on control flow\n", ID, i, PhyReg.Reg, ArgID);
      ...
      case OP_FILLREGISTER: {
        ...
        if (Value == RegState::UninitializedValue) {
          HadError |= true;
          Errors << fextl::fmt::format(
            "%{}: FillRegister expected %{} in Slot {}, but was undefined in at least one control flow path\n", ID, ExpectedValue,
            FillRegister->Slot);
        } else if (Value == RegState::ClobberedValue) {
          HadError |= true;
          Errors << fextl::fmt::format(
            "%{}: FillRegister expected %{} in Slot {}, but contents vary depending on control flow\n", ID, ExpectedValue, FillRegister->Slot);
        } else if (Value != ExpectedValue) {
          HadError |= true;
          Errors << fextl::fmt::format(
            "%{}: FillRegister expected %{} in Slot {}, but it actually contains %{}\n", ID, ExpectedValue, FillRegister->Slot, Value);
        }
        break;
      }
...
fextl::unique_ptr<FEXCore::IR::Pass> CreateRAValidation() { return fextl::make_unique<RAValidation>(); }
} // namespace FEXCore::IR::Validation
