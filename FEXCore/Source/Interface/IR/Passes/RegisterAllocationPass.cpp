...
namespace {
  ...
  void
  VirtualAddRegisterConflict(RegisterGraph* Graph, FEXCore::IR::RegisterClassType ClassConflict, uint32_t RegConflict, FEXCore::IR::RegisterClassType Class, uint32_t Reg) {
...
private:
  ...
  void
  RecursiveLiveRangeExpansion(FEXCore::IR::IRListView* IR, IR::NodeID Node, IR::NodeID DefiningBlockID, LiveRange* LiveRange, const fextl::unordered_set<IR::NodeID>& Predecessors, fextl::unordered_set<IR::NodeID>& VisitedPredecessors);

  FEXCore::IR::AllNodesIterator
  FindFirstUse(FEXCore::IR::IREmitter* IREmit, FEXCore::IR::OrderedNode* Node, FEXCore::IR::AllNodesIterator Begin, FEXCore::IR::AllNodesIterator End);
  FEXCore::IR::AllNodesIterator
  FindLastUseBefore(FEXCore::IR::IREmitter* IREmit, FEXCore::IR::OrderedNode* Node, FEXCore::IR::AllNodesIterator Begin, FEXCore::IR::AllNodesIterator End);

  std::optional<IR::NodeID>
  FindNodeToSpill(IREmitter* IREmit, RegisterNode* RegisterNode, IR::NodeID CurrentLocation, const LiveRange* OpLiveRange, int32_t RematCost = -1);
...
void ConstrainedRAPass::RecursiveLiveRangeExpansion(IR::IRListView* IR, IR::NodeID Node, IR::NodeID DefiningBlockID, LiveRange* LiveRange, const fextl::unordered_set<IR::NodeID>& Predecessors, fextl::unordered_set<IR::NodeID>& VisitedPredecessors) {
    ...
    for (auto [CodeNode, IROp] : IR->GetCode(BlockNode)) {
      ...
      for (uint8_t i = 0; i < NumArgs; ++i) {
        ...
        if (ArgNodeBlockID == BlockNodeID) {
          ...
          RecursiveLiveRangeExpansion(IR, ArgNode, ArgNodeBlockID, &ArgNodeLiveRange, Graph->BlockPredecessors[BlockNodeID], Graph->VisitedNodePredecessors[ArgNode]);
        }
      }
    }
  }
...
FEXCore::IR::AllNodesIterator
ConstrainedRAPass::FindFirstUse(FEXCore::IR::IREmitter* IREmit, FEXCore::IR::OrderedNode* Node, FEXCore::IR::AllNodesIterator Begin, FEXCore::IR::AllNodesIterator End) {
...
FEXCore::IR::AllNodesIterator
ConstrainedRAPass::FindLastUseBefore(FEXCore::IR::IREmitter* IREmit, FEXCore::IR::OrderedNode* Node, FEXCore::IR::AllNodesIterator Begin, FEXCore::IR::AllNodesIterator End) {
...
std::optional<IR::NodeID>
ConstrainedRAPass::FindNodeToSpill(IREmitter* IREmit, RegisterNode* RegisterNode, IR::NodeID CurrentLocation, const LiveRange* OpLiveRange, int32_t RematCost) {
